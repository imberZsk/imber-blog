...
使用 GSAP 的时候，这些 scrollTrigger 的 API 配置很关键
...
gsap 的 scrub 效果，也就是动画与滚动条绑定，也就是不会直接执行完动画，会跟着滚动条执行。
...
gsap 的 pin 效果，也就是滚动到某个位置时，元素会固定在页面不动。

[animation - pin](https://imber-animation.netlify.app/animations/pin)
...
### 滚动动画 + 叠层效果（Horizontal）

叠层效果，pinSpacing 为 false，左右滚动效果，snap 配置是啥

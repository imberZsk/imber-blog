...
3、 第三种是远程链接，使用的时候 `src='https://example.com/demo.png'`，为了安全，需要 [remotePatterns](https://nextjs.org/docs/app/api-reference/components/image#remotepatterns) 配置，也是必须设置 width 和 height，功能同第一种，远程图片也可以使用 `placeholder="blur"`，
`blurDataURL=base64` 来做一个渐进加载，这种方案只会多加载一张图，不会像小红书一样多加载很多图片请求，因为小红书是把每张图都做对应渐进加载，而不是同一张图

![image](/posts/responsive/image10.png)

当远程图片（如下图） width height 不确定的时候，可以使用图片的 fill 属性，它使图像扩展到父元素的大小，并且可以看到图片会有个absolute属性，所以父元素必须 “relative”， “fixed”， “absolute”，然后图片可以设置 objectFit 属性，来控制图片的显示方式。
...
还需要注意的是还设置了 sizes 和 srcset 属性，sizes 属性是用来设置图片的尺寸，srcset 属性是用来设置图片的源，这两个属性是用来控制图片的显示方式。

注意 Nextjs 对图片处理缩小是可以的，但是放大不行，所以下面的比如匹配上了 3840，但是原图最大宽度只有 200，那也只是显示 200 的图，不会放大

计算规则大约为视窗宽度（如 800px） \* DPR（如2） → 需要 1600px 物理宽度的图片，所以会匹配到 1920 的图片
...
背景：响应式的项目，在大屏的时候应该加载大图，小屏的时候应该加载小图，如果在小屏幕加载了一张很大的图，那会浪费资源

在响应式的时候不确定宽高的情况，但是比例或者显示的尺寸是定的，提供一张大图，然后通过这种方式就能实现图片的响应式

https://nextjs.org/docs/app/api-reference/components/image#sizes

sizes fill & srcset media 属性

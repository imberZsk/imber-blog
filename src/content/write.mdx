...
export default function VersionSortDemo() {
  ...
  function compareVersion(version1, version2) {
    // 1. 按 '.' 分割版本号
    const v1 = version1.split('.') // '1.2.3' → ['1', '2', '3']
    const v2 = version2.split('.') // '1.3.0' → ['1', '3', '0']
    ...
    for (let i = 0; i < length; i++) {
      // 4. 转为数字，缺失的段用 0 补齐
      const num1 = parseInt(v1[i] || 0) // v1[i] 不存在时用 0
      const num2 = parseInt(v2[i] || 0) // v2[i] 不存在时用 0

      // 5. 如果当前段不相等，直接返回差值
      if (num1 !== num2) {
        return num1 - num2 // 负数表示 version1 < version2
      }
      // 6. 相等则继续比较下一段
    }
...
export default function CurryDemo() {
  ...
  function curry(fn) {
    return function curried(...args) {
      // 如果参数长度大于等于原函数参数长度，直接调用原函数
      if (args.length >= fn.length) {
        return fn.apply(this, args)
      }
      // 否则返回一个新的函数，继续接收参数
      return function (...args2) {
        // 将新接收的参数拼接到已接收的参数中，并递归调用curried函数

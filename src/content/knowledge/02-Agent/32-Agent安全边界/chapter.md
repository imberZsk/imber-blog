...
```bash
cd demos/32-agent-safety
python3 main.py
```

`main.py` 把三道防线做成可见的拦截，核心是 `check_input` 和 `check_tool`：

- `check_input(message)` —— 输入侧，用 `BLOCK_PATTERNS` 拦 prompt 注入和密钥套取；
- `check_tool(tool_name, user_permissions)` —— 工具侧，用 `TOOL_PERMISSION` 拦越权调用。

跑完看到：三类攻击话术全被拦下，正常的查订单请求放行；同样想调 `delete_database`，普通用户因为没有 `admin:all` 权限被拦——权限在后端，不在 prompt。
...
`33-Dify入门.md` —— RAG 和 Agent 的原理都拆透了。接下来看怎么用 Dify、Coze 这类平台把这些能力快速搭起来，少写代码、快速验证。

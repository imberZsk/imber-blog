...
`main.py` 的核心是 `react_loop(question, max_steps)`，它把上面那个两步任务的每一轮 Thought / Action / Observation 都打印出来：

- 用 `state` 字典跨步骤传递中间结果（第 1 轮查到的订单存进去，第 2 轮取出来求和）；
- 三个决策分支分别对应「还没查订单→查」「查到了但没算总额→求和」「信息齐了→Final Answer」；
- `max_steps` 是步数上限，防止循环绕不出来。

跑一遍，你就看到一段完整的 ReAct trace，和上面的图一模一样。

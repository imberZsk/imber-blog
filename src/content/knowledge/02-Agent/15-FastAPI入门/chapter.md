...
```bash
cd demos/15-fastapi-intro
python3 main.py                  # 启动服务（标准库版，零依赖）
```
...
`main.py` 用标准库实现了上面四个要件，能直接对照看到本质：

- `do_GET` / `do_POST`：路由分发。
- `do_POST` 里读 Content-Length + json.loads：请求体解析。
- `_send_json`：统一响应出口。
- `invalid_json` / `missing_field` 分支：400 错误处理。

README 里给了逐行对应的 FastAPI 等价写法和 `pip install` 命令。先用标准库看清原理，生产再用 FastAPI 提效。

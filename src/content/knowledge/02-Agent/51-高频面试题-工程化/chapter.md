...
每题给三层：**一句话答**、**展开**、**加分项**。配套 `demos/51-interview-engineering/quiz.py` 能抽题自测。
...
**加分项**：强调"结构化日志 + requestId 串联"，而不是 print 一堆文本。（见 `demos/40-logging-observability`）
...
**加分项**：提"按相似度做语义缓存"和"模型分级路由"，比只说"加缓存"有深度。（见 `demos/41-cost-cache`）
...
**加分项**：点出"检索内容也可能是注入源"和"安全不能交给 prompt"。（见 `demos/32-agent-safety`）
...
**加分项**：强调"把模型输出当不可信数据处理"，这是工程师和调包侠的分水岭。（见 `demos/12-structured-output`）

### Q7：怎么评估一次 prompt/模型改动是变好还是变坏？
...
**加分项**：能说清"镜像 = 代码 + 依赖 + 环境"解决环境一致性，而不只是"容器化很流行"。（见 `demos/38-docker-basics`）
...
```bash
cd demos/51-interview-engineering
python3 quiz.py           # 随机抽 5 题
python3 quiz.py --all     # 通读全部
```

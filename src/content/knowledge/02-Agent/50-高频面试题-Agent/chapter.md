...
每题给三层：**一句话答**、**展开**、**加分项**。配套 `demos/50-interview-agent/quiz.py` 能抽题自测，还能 `--trace` 打印一个 ReAct 循环。
...
## 配套 demo：抽题自测 + 看 ReAct

```bash
cd demos/50-interview-agent
python3 quiz.py           # 随机抽 5 题
python3 quiz.py --all     # 通读全部
python3 quiz.py --trace   # 打印一个 ReAct 循环，照着记面试就能讲
```

## 下一篇

`51-高频面试题-工程化.md` —— RAG 和 Agent 讲完了，面试官还会问"这东西怎么稳定上线"。

...
```bash
cd demos/14-multi-turn-context
python3 main.py
```

`main.py` 跑一段 6 轮对话，`max_tokens=80` 故意设小好触发管理逻辑：

- `Conversation.history` + `add`：完整历史的累积，对应上面「手动维护列表」。
- `build_messages`：核心。没超预算就全量发；超了就调 `_summarize` 把旧历史压成摘要，只留最近 `keep_recent` 条原文。
- 输出里第 4→5 轮的变化最值得看：完整历史还在涨（8→10 条），但实际发给模型的从 8 条降到 5 条，因为旧的被摘要了。

`_summarize` 现在用规则抽关键词模拟，真实项目换成调模型即可，整体流程不变。

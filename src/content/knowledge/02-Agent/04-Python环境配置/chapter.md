...
```bash
cd demos/04-python-setup
python3 main.py
```

`main.py` 是一个「环境体检」脚本，不装任何东西，只把上面这几样的真实状态打印成一份诊断报告：

- `check_python_version()` —— 版本够不够 3.10
- `check_in_venv()` —— 用 `sys.prefix != sys.base_prefix` 判断你在不在虚拟环境里
- `check_pip()` —— pip 能不能用（能不能装依赖）
- `check_api_key()` —— 只看 Key 有没有设置，**绝不打印它的值**

如果你不在虚拟环境里跑，会看到虚拟环境那一项是 `[!!]` 警告，结论提示你先建 venv；激活后再跑就变 `[OK]`。这正是体检脚本该有的样子：把隐性的环境状态变成你能一眼看懂的结论。

...
```bash
cd demos/16-frontend-call-ai
python3 server.py
```

浏览器打开 http://127.0.0.1:8016，这是个能真实交互的页面：

- `index.html` 的 `state` + `syncUI`：状态集中管理，对应上面的状态表。
- `sendMessage`：完整处理 loading、成功、HTTP 错误、网络异常四条路径，重点看 `if (!res.ok)` 和 `catch` 怎么分开处理两类失败。
- `showError`：错误态带重试按钮。
- 后端 `server.py` 故意加了 0.6 秒延迟（让 loading 看得见），并且输入「报错」会返回 500（让你看错误态和重试）。

把后端关掉再发消息，能看到 `catch` 块的网络异常提示——这就是为什么网络层失败必须单独处理。

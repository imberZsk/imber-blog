...
```bash
cd demos/09-python-vs-js
python3 main.py
```

`main.py` 是同一段 AI 脚本逻辑的 Python 实现，每个关键处都用注释标了 JS 对应写法：

- `clean_messages()` —— 列表推导式 vs `.filter`
- `build_prompt()` —— `"\n".join` vs `.join`、f-string vs 模板字符串
- `fake_model()` / `handle()` —— `async def` + `asyncio.run` vs JS 自带事件循环
- `parse_json()` —— `json.loads` + `except 具体类型` vs `JSON.parse` + `catch`

跑完你会看到 `has_source` 是 `True`（大写），这就是 JSON 的 `true` 被转成 Python 布尔的直观证据。

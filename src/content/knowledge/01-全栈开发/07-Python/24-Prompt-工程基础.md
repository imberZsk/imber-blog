...
> 读完后，你应能完成以下任务：
> - 绘制“Python（24） - Prompt 工程基础 / 先给锚点：Prompt ≈ 给函数传参 + 写接口契约”的关键对象与数据流，解释“和大模型交互几乎一一对应——只不过「参数」是自然语言，「返回结构」要靠提示词去约束。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（24） - Prompt 工程基础 / 三种 role：system / user / assistant 怎么用”设计正常与异常输入，验证“这三个角色是所有 Prompt 工程的骨架，务必分清职责。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（24） - Prompt 工程基础 / temperature：随机性旋钮（类比 Math.random 的「强度」）”的最小代码或配置，检验“temperature 控制输出的随机程度。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先给锚点：Prompt ≈ 给函数传参 + 写接口契约**：和大模型交互几乎一一对应——只不过「参数」是自然语言，「返回结构」要靠提示词去约束。
- **三种 role：system / user / assistant 怎么用**：这三个角色是所有 Prompt 工程的骨架，务必分清职责。
- **temperature：随机性旋钮（类比 Math.random 的「强度」）**：temperature 控制输出的随机程度。
- **结构化输出：让模型返回能 JSON.parse 的数据**：这是前端转 AI 编程最实用的一招。
- **函数调用（function calling）：给模型注册一张「函数注册表」**：这是本篇最重要的概念，也是第 27 篇 Agent 的内核。
- **前端新手最容易踩的几个坑**：以为 API 有记忆。它无状态，多轮对话必须自己回填全部历史 messages（见第二节）。 -> 裸用提示词要 JSON 不做容错。 -> 把 Prompt 当咒语堆形容词。 -> 忘了 token 是要花钱的。

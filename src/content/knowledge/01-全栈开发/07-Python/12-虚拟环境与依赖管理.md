...
> 读完后，你应能完成以下任务：
> - 绘制“Python（12） - 虚拟环境与依赖管理 / 先建立直觉：虚拟环境 ≈ 项目专属的 node_modules”的关键对象与数据流，解释“类比：node 里你从不会想「这个包装到哪了」——因为它就在当前项目的 node_modules/，每个项目一份，互相隔离。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（12） - 虚拟环境与依赖管理 / 为什么非要虚拟环境？（不像 node 可以省）”设计正常与异常输入，验证“所以在 Python 里，「进任何项目先建/进虚拟环境」是和 npm install 同级的肌肉记忆，不是可选项。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（12） - 虚拟环境与依赖管理 / 方案一：venv + pip（标准库自带，最基础）”的最小代码或配置，检验“venv 是 Python 3 内置模块，pip 是默认装包工具。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先建立直觉：虚拟环境 ≈ 项目专属的 node_modules**：类比：node 里你从不会想「这个包装到哪了」——因为它就在当前项目的 node_modules/，每个项目一份，互相隔离。
- **为什么非要虚拟环境？（不像 node 可以省）**：所以在 Python 里，「进任何项目先建/进虚拟环境」是和 npm install 同级的肌肉记忆，不是可选项。
- **方案一：venv + pip（标准库自带，最基础）**：venv 是 Python 3 内置模块，pip 是默认装包工具。
- **方案二：poetry（≈ npm/yarn，统一管理）**：venv + pip 的痛点：装包、记依赖、建环境是三件分开的事，还得手动 pip freeze。
- **方案三：uv（新一代，主打一个快）**：uv 是用 Rust 写的新工具（来自 Astral，就是做 ruff 那家），目标是「一个工具搞定 venv + pip + poetry 全部职责」，最大特点是快得离谱（依赖求解和安装常比 pip/poetry 快一个数量级）。
- **三套方案怎么选？**：| 是否要额外装工具 | 否（标准库自带） | 是 | 是 |

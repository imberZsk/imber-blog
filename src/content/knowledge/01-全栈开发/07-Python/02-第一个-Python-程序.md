# Python（02） - 第一个 Python 程序

> 读完后，你应能完成以下任务：
> - 绘制“Python（02） - 第一个 Python 程序 / 先给个锚点：Python ≈ Node，不像 Java”的关键对象与数据流，解释“所以心智模型直接套 Node 就行：有解释器、没有编译产物、改完即跑。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（02） - 第一个 Python 程序 / 你的第一行代码”设计正常与异常输入，验证“注释用 #，不是 //（多行注释也没有 / /，靠每行开头 #，详见下文）”，输出首个偏差位置与回归测试结果。
> - 实现“Python（02） - 第一个 Python 程序 / 三种运行方式（都试一遍）”的最小代码或配置，检验“注意：有些系统里 python 指向老的 Python 2，要用 python3 hello.py。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先给个锚点：Python ≈ Node，不像 Java**：所以心智模型直接套 Node 就行：有解释器、没有编译产物、改完即跑。
- **你的第一行代码**：| console.log 是对象方法 | print 是内置函数 | Python 里它就是个全局函数，不挂在任何对象上 |
- **三种运行方式（都试一遍）**：注意：有些系统里 python 指向老的 Python 2，要用 python3 hello.py。
- **本篇重点：缩进即代码块（最该掰清楚的地方）**：冒号 : 是「块要开始了」的信号：def、if、else、for、while、class 等行尾都要加冒号，忘了直接语法报错。 -> 缩进就是花括号：往里缩一级 = 进入块，回到外层缩进 = 块结束。 -> 同一块里缩进必须完全一致：多一个空格、少一个空格都报 IndentationError。
- **再补几个「和 JS 不一样」的小点**：（你会看到三引号 """...""" 也常被当多行注释用，但它本质是「字符串」，正式用途是函数/模块的文档说明，后面学函数时再讲。
- **程序入口：if __name__ == "__main__"**：| require.main === module | __name__ == "__main__" | 判断「我是被直接运行，还是被别人 import」 |

...
> 读完后，你应能完成以下任务：
> - 绘制“Python（05） - 控制流与函数 / 前端锚点”的关键对象与数据流，解释“for 默认就是 for...of（遍历值，不是索引）。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（05） - 控制流与函数 / 控制流：先划清和 JS 的差异”设计正常与异常输入，验证“这是 Python 第一眼最大的不同。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（05） - 控制流与函数 / for：默认是 for...of，要索引用 enumerate”的最小代码或配置，检验“Python 的 for 没有 for (let i = 0; ...) 这种 C 式写法，默认就是遍历值，等价于 JS 的 for...of。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **前端锚点**：for 默认就是 for...of（遍历值，不是索引）。
- **控制流：先划清和 JS 的差异**：这是 Python 第一眼最大的不同。
- **函数 def：重点章节**：lambda 是 Python 的「箭头函数」，但只能放一个表达式，不能写多行逻辑——所以 Python 里写复杂逻辑一律用 def，lambda 只用于 sorted(key=...) 这类一次性小函数
- **args 和 kwargs：收集任意参数**：这是本篇最该花时间的概念，也是读第三方库源码（到处是 def foo(args, kwargs)）的前提。
- **前端新手最易踩的坑**：Python 缩进是语法。
- **没有花括号，缩进就是代码块**：这是 Python 第一眼最大的不同。

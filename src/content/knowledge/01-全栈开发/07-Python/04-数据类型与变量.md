...
> 读完后，你应能完成以下任务：
> - 绘制“Python（04） - 数据类型与变量 / 变量：没有 let/const，直接赋值”的关键对象与数据流，解释“没有分号（加了也不报错，但不符合习惯，不要加）。 -> 命名习惯是 snake_case（下划线），不是 JS 的 camelCase。 -> 没有 const。Python 没有真正的常量机制，约定俗成用全大写表示"别改我"：MAX_SIZE = 100。这只是约定，语法上还是能改。 -> Python 是动态类型，这点和 JS 一样：一个变量可以先存数字再存字符串（虽然不建议）。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（04） - 数据类型与变量 / 数字：int 和 float 是两种类型”设计正常与异常输入，验证“/ 永远得到 float：6 / 2 在 Python 里是 3.0 不是 3。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（04） - 数据类型与变量 / 字符串 str：没有反引号，用 f-string”的最小代码或配置，检验“求长度是 len(s) 不是 s.length。 -> 不会自动转类型："年龄" + 18 在 JS 里得到 "年龄18"，在 Python 里直接报错（TypeError）。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先建立一个总锚点**：| JavaScript | Python | 一句话区别 |
- **变量：没有 let/const，直接赋值**：没有分号（加了也不报错，但不符合习惯，不要加）。 -> 命名习惯是 snake_case（下划线），不是 JS 的 camelCase。 -> 没有 const。Python 没有真正的常量机制，约定俗成用全大写表示"别改我"：MAX_SIZE = 100。这只是约定，语法上还是能改。 -> Python 是动态类型，这点和 JS 一样：一个变量可以先存数字再存字符串（虽然不建议）。
- **数字：int 和 float 是两种类型**：/ 永远得到 float：6 / 2 在 Python 里是 3.0 不是 3。
- **字符串 str：没有反引号，用 f-string**：求长度是 len(s) 不是 s.length。 -> 不会自动转类型："年龄" + 18 在 JS 里得到 "年龄18"，在 Python 里直接报错（TypeError）。
- **list：长得像 Array，方法名不一样**：| arr.includes(x) | x in arr | 是否包含 |
- **dict：JS 的 Object 和 Map 合二为一**：没有点号访问：JS 习惯 user.name，Python 只能 user["name"]。 -> 取不存在的键会抛 KeyError，不是返回 undefined。

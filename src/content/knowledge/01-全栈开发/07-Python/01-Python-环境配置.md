# Python（01） - Python 环境配置

> 读完后，你应能完成以下任务：
> - 绘制“Python（01） - Python 环境配置 / 先建立总类比”的关键对象与数据流，解释“⚠️ 边界（哪里不一样）：上面是「建立直觉」，但 pip 和 npm 有一个致命差异——npm 默认装到当前项目的 node_modules（本地隔离），而 pip 默认装到全局。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（01） - Python 环境配置 / Python 解释器：装哪个版本”设计正常与异常输入，验证“类比：就像现在没人写 var 一样，Python 2 是历史包袱，直接忽略。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（01） - Python 环境配置 / python 还是 python3？（第一个坑）”的最小代码或配置，检验“所以养成习惯敲 python3 / pip3 最稳。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先建立总类比**：⚠️ 边界（哪里不一样）：上面是「建立直觉」，但 pip 和 npm 有一个致命差异——npm 默认装到当前项目的 node_modules（本地隔离），而 pip 默认装到全局。
- **Python 解释器：装哪个版本**：类比：就像现在没人写 var 一样，Python 2 是历史包袱，直接忽略。
- **venv 虚拟环境：最重要的一节**：Python 的 pip install 默认装到全局（系统级或用户级），结果是：
- **pip：装包与依赖清单**：| npm install requests@2.31.0 | pip install requests==2.31.0 | 装指定版本（注意是 ==） |
- **IDE：用 VS Code 或 PyCharm**：| 选项 | 类比 | 适合谁 |
- **最小上手验证（5 分钟跑通）**：💡 注意上面那行 if __name__ == "__main__":——它是 Python 的入口惯例，含义是「这个文件被直接运行时才执行，被别的文件 import 时不执行」。

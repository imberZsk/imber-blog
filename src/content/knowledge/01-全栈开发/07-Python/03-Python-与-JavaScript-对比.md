# Python（03） - Python 与 JavaScript 对比

> 读完后，你应能完成以下任务：
> - 绘制“Python（03） - Python 与 JavaScript 对比 / 核心思路”的关键对象与数据流，解释“和你从 JS 转 Java 不同：Java 是"强类型 + 编译检查"，处处和 JS 别扭；”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（03） - Python 与 JavaScript 对比 / 变量和类型”设计正常与异常输入，验证“True / False / None 首字母大写。 -> int 和 float 是分开的两种类型（JS 只有一个 number）。 -> 没有 undefined，空值只有 None 一个。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（03） - Python 与 JavaScript 对比 / 变量声明”的最小代码或配置，检验“全大写 AGE 只是约定告诉别人"别改我"，语言层面照样能改。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **核心思路**：和你从 JS 转 Java 不同：Java 是"强类型 + 编译检查"，处处和 JS 别扭；
- **变量和类型**：True / False / None 首字母大写。 -> int 和 float 是分开的两种类型（JS 只有一个 number）。 -> 没有 undefined，空值只有 None 一个。
- **函数**：因为默认值只在定义时创建一次，多次调用会共享同一个 list，导致诡异 bug。
- **类和对象（最大的坑：self）**：self 不是 this。 -> 构造方法固定叫 __init__，不是 constructor。 -> 创建对象不用 new，直接 User("Tom", 18)。 -> 访问成员一律 self.xxx，漏写 self. 会被当成局部变量。
- **控制流（和 JS 像，但有几个坑）**：| && \|\| ! | and or not | 用英文单词，不是符号 |
- **模块与包（≈ import / npm）**：边界：JS 的依赖装在项目本地 node_modules；

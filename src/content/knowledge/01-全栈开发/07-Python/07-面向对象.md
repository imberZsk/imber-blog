...
> 读完后，你应能完成以下任务：
> - 绘制“Python（07） - 面向对象 / 边界一：self 不是 this，它必须"显式出现"”的关键对象与数据流，解释“每个实例方法的第一个参数必须是 self（名字是约定，理论上可改，但永远别改）。 -> 调用时 tom.greet() 会被 Python 自动翻译成 User.greet(tom)，tom 被塞进 self。 -> 访问实例属性必须写 self.name，不能像 JS 那样在方法里直接写 name。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Python（07） - 面向对象 / 边界二：类属性 vs 实例属性（最隐蔽的坑）”设计正常与异常输入，验证“前端容易踩坑，是因为 JS 写在 class 体里的 count = 0 默认是实例字段，而 Python 写在 class 体里的是类属性——同样的位置，含义相反。”，输出首个偏差位置与回归测试结果。
> - 实现“Python（07） - 面向对象 / 继承：和 extends 几乎一样，super 略有区别”的最小代码或配置，检验“Python super().__init__() 要自己手动调，否则父类的 __init__ 不会跑（JS 里如果子类写了 constructor，不调 super 会直接报错，Python 不报错只是不执行，更隐蔽）。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **边界一：self 不是 this，它必须"显式出现"**：每个实例方法的第一个参数必须是 self（名字是约定，理论上可改，但永远别改）。 -> 调用时 tom.greet() 会被 Python 自动翻译成 User.greet(tom)，tom 被塞进 self。 -> 访问实例属性必须写 self.name，不能像 JS 那样在方法里直接写 name。
- **边界二：类属性 vs 实例属性（最隐蔽的坑）**：前端容易踩坑，是因为 JS 写在 class 体里的 count = 0 默认是实例字段，而 Python 写在 class 体里的是类属性——同样的位置，含义相反。
- **继承：和 extends 几乎一样，super 略有区别**：Python super().__init__() 要自己手动调，否则父类的 __init__ 不会跑（JS 里如果子类写了 constructor，不调 super 会直接报错，Python 不报错只是不执行，更隐蔽）。
- **静态方法与类方法：对应 JS 的 static**：@classmethod 是 Python 特有，拿得到类本身 cls，最常见用途是提供"工厂方法"（如 User.from_json(data)）。
- **魔术方法（dunder）：定制对象的"内置行为"**：边界提醒：JS 不支持运算符重载（a + b 行为固定）。
- **一个易混淆的小语法：没有 private，用下划线"约定"**：_balance：纯君子协定，等于 TS 里加了 private 但没编译检查——能访问，只是"你不该访问"。

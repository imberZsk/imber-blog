# Java（28） - xxl-job 分布式定时任务

> 读完后，你应能完成以下任务：
> - 绘制“Java（28） - xxl-job 分布式定时任务 / 先回忆：前端是怎么做"定时"的”的关键对象与数据流，解释“在前端/单机脚本里这没问题，但搬到后端集群就全是坑。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（28） - xxl-job 分布式定时任务 / 为什么后端不能直接用单机 cron”设计正常与异常输入，验证“后端服务为了高可用，从来不是只跑一个进程。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（28） - xxl-job 分布式定时任务 / 坑 2：改时间要重新发版”的最小代码或配置，检验“cron 表达式写死在代码里。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先回忆：前端是怎么做"定时"的**：在前端/单机脚本里这没问题，但搬到后端集群就全是坑。
- **为什么后端不能直接用单机 cron**：后端服务为了高可用，从来不是只跑一个进程。
- **xxl-job 的核心架构：调度中心 + 执行器**：| 角色 | 是什么 | 前端类比 |
- **执行器是怎么接入的：XxlJobConfig**：adminAddresses：执行器知道调度中心在哪，启动时主动连过去说"我是 demo-billing 的一个 Pod，有任务可以派给我"。 -> appname：执行器分组名。 -> @ConditionalOnProperty（见第 8 课注解、第 5 课 Bean）：本地 xxl.enabled=false 就不启动执行器，省得本地连不上调度中心一直报错。
- **@XxlJob：把一个方法变成"可被调度的任务"**：| 返回 ReturnT.SUCCESS / FAIL | 没有对应物——后端要主动汇报成败，调度中心据此决定是否重试/报警 |
- **cron 表达式：和前端 node-cron 几乎一样**：注意：xxl-job 用的是 Quartz 风格的 cron，"周"这一位是 1-7、1 代表周日（和 Linux crontab 的 0-7、0=周日 不一样）。

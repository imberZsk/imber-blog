# Java（16） - Spring 事务与 @Transactional

> 读完后，你应能完成以下任务：
> - 绘制“Java（16） - Spring 事务与 @Transactional / 事务边界应该放在哪里”的关键对象与数据流，解释“事务通常放在 Service 的公开方法上，因为 Service 表达一个完整业务动作，能够覆盖多个 Mapper 调用。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（16） - Spring 事务与 @Transactional / 为什么注解能控制提交和回滚”设计正常与异常输入，验证“方法正常返回就提交，抛出符合规则的异常就回滚。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（16） - Spring 事务与 @Transactional / 回滚规则不要靠猜”的最小代码或配置，检验“如果业务必须返回错误对象，也要先抛到事务边界之外再转换，或明确调用”，输出命令、结果与 Diff，并说明不适用边界。
...
- **事务边界应该放在哪里**：事务通常放在 Service 的公开方法上，因为 Service 表达一个完整业务动作，能够覆盖多个 Mapper 调用。
- **为什么注解能控制提交和回滚**：方法正常返回就提交，抛出符合规则的异常就回滚。
- **回滚规则不要靠猜**：如果业务必须返回错误对象，也要先抛到事务边界之外再转换，或明确调用
- **传播行为怎么选**：| 传播行为 | 含义 | 典型用途 | 风险 |
- **隔离级别与并发更新**：随后检查影响行数是否为 1。
- **六类不生效问题**：类内部调用：this.transactionalMethod() 绕过代理。 -> 异常被捕获：方法正常返回，代理执行提交。 -> 回滚类型不匹配：抛出受检异常，但没有配置 rollbackFor。 -> 对象不是 Spring Bean：手动 new 出来的对象没有事务代理。

...
> 读完后，你应能完成以下任务：
> - 绘制“Java（08） - 注解与反射 / 注解是什么？”的关键对象与数据流，解释“注解就是代码里的 @xxx 标记。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（08） - 注解与反射 / 用前端类比理解”设计正常与异常输入，验证“或者更通俗的类比：注解像快递单上的标签——"易碎"、"加急"、"冷藏"。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（08） - 注解与反射 / @RequestParam vs @RequestBody（前端联调必懂）”的最小代码或配置，检验“这是前后端联调最常见的问题之一。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **demo 常见注解速查**：| @Service | 标记这是个 Service，交给框架管理 |
- **@RequestParam vs @RequestBody（前端联调必懂）**：这是前后端联调最常见的问题之一。
- **Lombok 注解：少写样板代码**：@Data 帮你把 getter/setter 全生成了，所以 demo 的实体类看起来很干净，只有字段。

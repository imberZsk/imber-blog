...
> 读完后，你应能完成以下任务：
> - 绘制“Java（11） - MVC 三层架构 / 先用前端的话讲清楚：你早就在分层了”的关键对象与数据流，解释“写 React/Vue 的时候，一个稍微像样的项目你绝不会把「发请求 + 改状态 + 渲染按钮」全塞进一个组件里。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（11） - MVC 三层架构 / 用 demo-basic 示例链路串一遍”设计正常与异常输入，验证“前端类比：这正是你写的 Vue 组件里 @click="userStore.fetchUser(id)"——组件只负责把事件转给 store，不在 里写业务判断。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（11） - MVC 三层架构 / 第 2 层 Service —— 真正干活的大脑”的最小代码或配置，检验“这就是「大脑」该干的：决策和编排。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先用前端的话讲清楚：你早就在分层了**：| 页面组件 / View | 收集输入、展示结果 | Controller | 收 HTTP 请求、返响应 |
- **用 demo-basic 示例链路串一遍**：前端类比：这正是你写的 Vue 组件里 @click="userStore.fetchUser(id)"——组件只负责把事件转给 store，不在 里写业务判断。
- **为什么非要分这三层？**：这跟你把请求逻辑抽进 useUser() hook 而不是每个组件各写一遍 fetch，是一模一样的动机。
- **demo 项目里层是怎么落到目录上的**：包名里的 controller / service / mapper 不是装饰，是团队约定：看到包名就知道这个类属于哪一层、该写什么、不该写什么。
- **动手判断：这段代码该写在哪一层？**：Service。这是业务校验，是「业务知识」。 -> Controller。HTTP 方法/协议属于收发层，业务逻辑一行不用动。 -> Mapper（或 Service 里构造 QueryWrapper 时）。 -> Service。用 BeanUtils.copyProperties(organization, organizationOut) 做 实体→出参 的裁剪，是业务层的数据整形职责。
- **本课小结**：Service 是大脑：业务校验（抛 BusinessException）、流程编排（如 buildOrganizationTree 攒多个数据源）、事务边界（@Transactional）、实体→出参转换（BeanUtils.copyProperties）都在这层。

...
> 读完后，你应能完成以下任务：
> - 绘制“Java（31） - 实战——给现有接口加 Redis 缓存 / 先回顾：为什么要缓存”的关键对象与数据流，解释“你可以先粗暴地理解为：一个跑在服务器上的、超大号的、所有 Java 进程共享的 Map ，数据存在内存里所以读写极快（微秒级），而且支持给每个 key 设过期时间。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（31） - 实战——给现有接口加 Redis 缓存 / Cache Aside 模式：业界最常用的缓存套路”设计正常与异常输入，验证“注意：更新时是「删除缓存」，不是「更新缓存」。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（31） - 实战——给现有接口加 Redis 缓存 / 读流程（查询时）”的最小代码或配置，检验“一句话：先查缓存，命中就返回；”，输出命令、结果与 Diff，并说明不适用边界。
...
demo 的 `RouteCacheService`（文件：`demo-basic/demo-basic-service/src/main/java/com/example/platform/basic/service/service/RouteCacheService.java`）缓存的是「一条标准路线的坐标点列表」——这种数据典型的「读多写少」：路线一旦画好很少改，但每次司机导航、运单展示都要读。
...
private List<Coord> findRoutePointsFromCache(Integer routeId) {
    ...
    // 命中：把存储的字符串解码回坐标列表（demo 用 coordEncoder 把坐标压成短字符串再存，省内存）
...
## 3.3 写：demo 这里用的是「更新缓存」而非「删缓存」

讲到这你可能以为 demo 写流程就是「改库 → 删缓存」。但翻开匿名化示例代码，它走的是另一条路：**新增/更新路线时，顺手把新坐标点写回缓存。** 看 `create` 和 `update` 的尾巴都调了同一个 `cacheRoutePoints`：
...
为什么 demo 敢这么写，而第二节又说推荐「删缓存」？这正好对照第二节那张表，把取舍讲透：

| | 第二节的「删缓存」（推荐默认） | demo 这里的「更新缓存」 |
|---|---|---|
| 适用前提 | 通用场景，尤其是并发更新多 | 写入串行、且新值现成（`prepare` 阶段已编码好） |
| 好处 | 简单、不会写入旧值 | 写完立刻命中缓存，省掉「下次读再查库回填」那一跳 |
| 风险 | 下次读有一次回源 | 并发更新时可能把缓存写成旧值（时序问题） |

demo 这个场景里，标准路线的新增/更新都在事务内串行执行、且新坐标点是当前请求算出来的，所以「更新缓存」的并发风险很低，换来的是写完即热、读路径更短。**这不是反例，而是「理解原理后按场景灵活选型」的匿名化案例。**

> 提醒：如果你拿不准，默认按第二节的「删缓存」写，最不容易出错。「更新缓存」要用得对，前提是你能确认写入是串行的、且新值就是最终值。demo 删除路线时不清缓存、纯靠过期兜底，也是同样的权衡——能容忍最多 1 小时的旧数据。

---

# 四、给第 30 课的接口加缓存（前后对比）

第 30 课我们写了一个查询接口（这里以「根据公司 ID 查公司详情」为例，沿用第 04/05 课的 `OrganizationService` 风格）。下面把它从「无缓存」改造成「Cache Aside」。
...
public class OrganizationService {
    ...
    // 加缓存前：每次都查库
    // 入参 organizationId：公司 ID
    public OrganizationVo getById(Integer organizationId) {
        Organization organization = organizationMapper.selectById(organizationId); // 每次都打 MySQL
        if (organization == null) {
            throw new BusinessException("公司不存在"); // 见第 07 课异常处理
        }
        return OrganizationVo.from(organization);
    }
}
```
...
public class OrganizationService {
    ...
    // 缓存 key 前缀，最终形如 "basic:organization:info:1001"
    private static final String CACHE_KEY_PREFIX = "basic:organization:info:";
    ...
    // 入参 organizationId：公司 ID，也是缓存 key 的业务标识
    public OrganizationVo getById(Integer organizationId) {
        String key = CACHE_KEY_PREFIX + organizationId;
        ...
        OrganizationVo vo = OrganizationVo.from(organization);
    ...
    // 更新公司信息：改库 → 删缓存
    // 入参 organization：待更新的公司实体
    public void update(Organization organization) {
        organizationMapper.updateById(organization); // 先写库
        String key = CACHE_KEY_PREFIX + organization.getId();
        redis.delete(key); // 再删缓存，下次读自动加载最新值
    }

    // 把 VO 序列化成 JSON 字符串存进 Redis（Redis 只认字符串）
    private String toJson(OrganizationVo vo) {
        try {
    ...
    private OrganizationVo parse(String json) {
        try {
            return objectMapper.readValue(json, OrganizationVo.class);
        } catch (JsonProcessingException e) {
...
看 demo 的 key：`line:std_route:points:1001`，公司的：`basic:organization:info:1001`。规律是：

```text
业务域  :  实体     :  维度     :  业务ID
basic   :  organization  :  info     :  1001
...
> 前端类比：跟你给 `react-query` 设计 `queryKey` 一模一样——`['organization', 'info', id]`。带上业务维度，保证不同数据不串。
...
这一课先建立 Cache Aside 的正确心智，这两个进阶问题知道名字和大致思路即可，真正落地时再深入。demo 里也确实用到了分布式锁（`LockService`），属于后面的话题。
...
- **demo 示例实践**：`RouteCacheService` 的**读流程**是标准范本——拆成「编排方法 `findRoutePoints` + 查缓存/查库/写缓存三个小方法」，key 前缀 `line:std_route:points:`，过期 1 小时。它的**写流程**用的是「更新缓存」而非「删缓存」（删除路线则纯靠过期兜底），是「按场景灵活选型」的工程取舍，不是模板。
- **key 设计**：业务前缀 + 冒号分层 + 全业务标识，例如 `basic:organization:info:1001`。
- **过期时间**：永远要设（兜底防漏删），且加随机抖动避免缓存雪崩。
- **性能权衡**：用「极短的数据延迟」换「大幅性能提升」，适合能容忍最终一致的数据，不适合余额/库存这类强一致场景。
- **进阶坑**：缓存穿透（缓存空值）、缓存击穿（加锁回填），先记住名字和思路。

**下一课预告**：第 32 课《实战——加缓存后的数据一致性陷阱》。这一课我们埋了好几个伏笔（先删还是后删、删失败怎么办、并发更新），下一课就来系统拆解「缓存与数据库如何保持一致」，并看 demo 是怎么用分布式锁兜住这些坑的。

# 八、总结

- **先回顾：为什么要缓存**：| React.useMemo / useCallback | 组件内存 | 避免重复计算 |
- **Cache Aside 模式：业界最常用的缓存套路**：注意：更新时是「删除缓存」，不是「更新缓存」。
- **demo 匿名化案例：车线标准路线的坐标点缓存**：demo 的 RouteCacheService（文件：demo-basic/demo-basic-service/src/main/java/com/example/platform/basic/service/service/RouteCacheService.java）缓存的是「一条标准路线的坐标点列表」——这种数据典型的「读多写少」：路线一旦画好很少
- **给第 30 课的接口加缓存（前后对比）**：| 每次请求是否查库 | 是 | 仅首次/过期后查库，其余命中 Redis |
- **缓存 key 设计与过期时间：两个最容易踩坑的点**：| 加业务前缀 | Redis 全集群共享，前缀避免和别的模块撞 key | 直接用 1001 当 key（必撞） |
- **还要知道的两个坑（生产必遇）**：| 问题 | 是什么 | 简单应对 |

...
- **切入点：你早就被语言坑过**：| obj2 = obj1 引用复制改一动二 | 对象/集合赋值是引用，改一动二 | 三 |
- **== 比引用：字符串/对象比较必须用 .equals()**：这是前端转 Java 头号必踩坑，几乎人人栽一次。
- **包装类型 Integer：自动拆箱 NPE 与缓存陷阱**：这是 Java 独有、JS 完全没有对应物的坑，因为 JS 的 number 只有一种。
- **对象/集合赋值是引用：b = a 不复制内容**：这个坑 JS 程序员完全有共鸣——const b = a 对对象/数组只是复制引用，改一个动另一个。
- **数字坑：整数溢出、整数除法截断、浮点误差**：Java 的 int 是 32 位整数，有上限，还有「整数除法截断」这个 JS 完全没有的坑。
- **附赠几个高频小坑（认脸即可）**：这些不展开，先混个眼熟，遇到不至于懵：

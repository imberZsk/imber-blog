# Java（09） - Spring Boot 是什么

> 读完后，你应能完成以下任务：
> - 绘制“Java（09） - Spring Boot 是什么 / 先回答：没有 Spring Boot 之前有多痛”的关键对象与数据流，解释“对象一多就是一张盘根错节的「依赖网」，改一处牵一片。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（09） - Spring Boot 是什么 / IoC：控制反转——把「谁来 new」的权力交出去”设计正常与异常输入，验证“IoC（Inversion of Control，控制反转） 是整套体系的灵魂。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（09） - Spring Boot 是什么 / 前端类比”的最小代码或配置，检验“这就是控制反转——你交出了控制权，换来了不用操心。”，输出命令、结果与 Diff，并说明不适用边界。
...
## 4.1 前端类比：DI ≈ Vue 的 provide/inject，但更彻底
...
> **常见疑问：为什么提倡用 DI 而不是自己 new？** 因为容器统一管理后：① 默认单例，省内存；② 依赖关系由框架维护，改依赖不用改一堆 `new`；③ 测试时可以轻松「换成假的 Bean」（Mock），这对写单元测试很关键（见第 08 课测试相关）。
...
- **先回答：没有 Spring Boot 之前有多痛**：对象一多就是一张盘根错节的「依赖网」，改一处牵一片。
- **IoC：控制反转——把「谁来 new」的权力交出去**：IoC（Inversion of Control，控制反转） 是整套体系的灵魂。
- **容器与 Bean：框架管理的对象池**：Bean 就是「由 Spring 容器创建并管理的对象」。
- **DI：依赖注入——揭晓 @Autowired 之谜**：DI（Dependency Injection，依赖注入） 是 IoC 的具体实现手段：
- **起步依赖 starter：一句话装好一整套**：当你要做一个项目，你不会一个个手动装 webpack、babel-loader、@babel/preset-env、css-loader…… 而是装一个「全家桶」让它把配套依赖和默认配置一并带来。
- **自动配置：约定优于配置**：starter 带来依赖只是第一步，自动配置（Auto Configuration） 才是 Spring Boot 真正「省心」的地方。

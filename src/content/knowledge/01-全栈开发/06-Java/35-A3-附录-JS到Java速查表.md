...
## 8.2 方法重载（Java 特有，≈ TS 函数重载但真存在多个实现）
...
# 十五、Web 后端：Spring Boot ≈ 带强类型的 Express
...
- **变量与基本类型**：| let x = 1; x = "hi" | ❌ 编译报错（⚠️ 类型定死不能换） |
- **字符串操作**：| str.length | str.length() | ⚠️ 是方法要加括号，不是属性 |
- **数组 / 集合**：⚠️ Java 里「数组」和「List」是两个东西（JS 只有一个 Array）：
- **高阶函数：map / filter / reduce → Stream**：这是前端最能无缝迁移的地方之一。
- **对象 / 字典（Map）**：| obj.key / obj["key"] | map.get("key") | ⚠️ 用 get，不能用点号也不能用下标 |

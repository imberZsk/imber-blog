# Java（03） - Java 与 JavaScript 对比

> 读完后，你应能完成以下任务：
> - 绘制“Java（03） - Java 与 JavaScript 对比 / 核心思路”的关键对象与数据流，解释“Java 就是把类型检查做到极致，而且不允许绕过。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（03） - Java 与 JavaScript 对比 / 变量和类型”设计正常与异常输入，验证“Java 类型在前面：类型 变量名 = 值;（TS 是 变量名: 类型）”，输出首个偏差位置与回归测试结果。
> - 实现“Java（03） - Java 与 JavaScript 对比 / 函数 vs 方法”的最小代码或配置，检验“String[] args 不能省略，否则 JVM 找不到 main 方法”，输出命令、结果与 Diff，并说明不适用边界。
...
- **核心思路**：Java 就是把类型检查做到极致，而且不允许绕过。
- **变量和类型**：Java 类型在前面：类型 变量名 = 值;（TS 是 变量名: 类型）
- **函数 vs 方法**：String[] args 不能省略，否则 JVM 找不到 main 方法
- **数组和集合**：| JavaScript | Java |
- **类和对象**：没有 constructor 关键字，构造方法名 = 类名 -> 成员变量要加访问修饰符（private/public）
- **控制流（和 JS 基本一样）**：| JS | Java |

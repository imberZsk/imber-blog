...
> 读完后，你应能完成以下任务：
> - 绘制“Java（10） - 创建第一个项目 —— Spring Boot 项目结构详解 / 先建立一个总览类比”的关键对象与数据流，解释“你在前端创建一个项目，脑子里有一张固定的图：package.json 管依赖、vite.config.ts 管构建配置、src/ 放源码、main.ts 是入口。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（10） - 创建第一个项目 —— Spring Boot 项目结构详解 / Maven 项目骨架：约定优于配置”设计正常与异常输入，验证“这是 demo-basic 项目里负责"基础数据服务"的模块。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（10） - 创建第一个项目 —— Spring Boot 项目结构详解 / 多模块项目：Monorepo 的 Java 版”的最小代码或配置，检验“打开 demo-basic 你会发现它不是单个模块，而是一个父项目带四个子模块，这正是前端 monorepo（pnpm workspace / lerna）的思路。”，输出命令、结果与 Diff，并说明不适用边界。
...
```text
demo-basic-service/                    # 一个 Maven 模块
├── pom.xml                           # 模块的"package.json"
└── src/
    ├── main/                         # 主代码（会打进最终的 jar）
    │   ├── java/                     # Java 源码根（≈ src/）
    │   │   └── com/example/platform/basic/service/
    │   │       ├── service/          # 业务逻辑层
    │   │       ├── repository/       # 数据访问封装
...
> 示例路径来自 `demo-basic/demo-basic-service`。这是 demo-basic 项目里负责"基础数据服务"的模块。
...
<modules>
    <module>demo-basic-common</module>   <!-- 公共常量/工具 -->
    <module>demo-basic-service</module>  <!-- 业务逻辑实现 -->
    <module>demo-basic-client</module>   <!-- 对外暴露的接口定义（给别的服务调用）-->
    <module>demo-basic-biz</module>      <!-- 启动入口 + Controller -->
</modules>
```

| 前端 monorepo | demo-basic 多模块 |
| --- | --- |
| 根 `package.json`（workspaces 字段） | 父 `pom.xml`（`<modules>` + `packaging=pom`） |
| `packages/utils`（公共包） | `demo-basic-common`（公共常量/工具） |
| `packages/core`（核心逻辑） | `demo-basic-service`（业务实现） |
| `packages/sdk`（给外部用的包） | `demo-basic-client`（对外接口定义） |
| `apps/web`（可启动的应用） | `demo-basic-biz`（带启动类的应用层） |

模块之间通过依赖串起来。比如 `demo-basic-biz/pom.xml` 里声明它依赖 `service` 模块：

```xml
<!-- 文件：demo-basic-biz/pom.xml -->
<dependencies>
    <dependency>
        <groupId>com.example.platform</groupId>
        <artifactId>demo-basic-service</artifactId>   <!-- biz 依赖 service -->
        <version>5.1.0-SNAPSHOT</version>
    </dependency>
    ...
</dependencies>
...
```text
请求 → demo-basic-biz(Controller) → demo-basic-service(Service/业务)
     → demo-basic-service(Mapper) → MySQL
```
...
<groupId>com.example.platform</groupId>      <!-- ≈ npm 的 scope，如 @example -->
<artifactId>demo-basic</artifactId>         <!-- ≈ 包名 -->
...
<parent>
    <groupId>com.example.platform</groupId>
    <artifactId>demo-parent</artifactId>     <!-- 公司级父 pom，统一管 Spring Boot 版本等 -->
    <version>5.1.0-SNAPSHOT</version>
</parent>
```

前端没有完全对应的概念，最接近的是 `tsconfig.json` 的 `"extends"`：继承一份基础配置，自己只写差异部分。`demo-parent` 帮所有子项目统一规定了 Spring Boot 版本、编译插件、编码格式，避免每个项目各写一遍。

## 4.3 dependencies = dependencies

```xml
<!-- 文件：demo-basic/pom.xml，根 <dependencies> -->
<dependencies>
    <dependency>
...
spring:
  application:
    name: demo-basic        # 等价 JSON: { "spring": { "application": { "name": "demo-basic" } } }
```

## 5.2 看 demo-basic 的示例 bootstrap.yml

```yaml
# 文件：demo-basic-biz/src/main/resources/bootstrap.yml
spring:
  application:
    name: demo-basic-${user.name}-local   # ${user.name} 是占位符，会被替换成系统用户名
  profiles:
...
```yaml
# 文件：demo-basic-biz/src/main/resources/bootstrap.yml（节选）
---
spring:
  profiles: dev                          # 这一段只在 dev 环境生效
apollo:
  meta: http://apollo-api.dev.example.com:8080   # dev 用开发环境配置中心

---
spring:
  profiles: prod                      # 这一段只在生产环境生效
apollo:
  meta: http://apollo.prod.example.com:8000     # 生产用生产配置中心
```

| 前端 | Spring Boot |
| --- | --- |
| `.env.development` / `.env.production` | `application-dev.yml` / `application-prod.yml`（或一个文件用 `---` 分段） |
| `NODE_ENV=production` 切换环境 | `spring.profiles.active=prod` 切换 profile |
| `import.meta.env.VITE_X` 读变量 | `@Value("${apollo.meta}")` 或配置类读变量 |

## 5.4 bootstrap.yml vs application.yml

demo-basic 这里只有 `bootstrap.yml`，因为它把大部分配置放到了**远程配置中心 Apollo**（类似一个集中管理所有服务配置的后台）。两个文件的分工：
...
每个 Spring Boot 应用有且只有一个启动类，它就是 `src/main.ts`。看 demo-basic 的示例启动类：

```java
// 文件：demo-basic-biz/src/main/java/com/example/platform/basic/biz/DemoBasicAdminApplication.java
package com.example.platform.basic.biz;
...
@EnableFeignClients(basePackages = {"com.example.platform"})
// @ComponentScan：指定要扫描哪些包下的 Spring 组件（@Service/@Controller 等）
@ComponentScan({"com.example.platform"})
...
```bash
java -jar demo-basic-biz.jar
        │
        ▼
  main() → SpringApplication.run(...)
        │
        ├─ 1. 读 bootstrap.yml → 连 Apollo 配置中心，拉远程配置
        ├─ 2. @ComponentScan 扫描 com.example.platform 下所有组件
...
demo-basic-service 的示例目录展示了一套标准分层。这套约定不是 Maven 强制的，而是**团队/Spring 社区的共识**，类似前端约定 `components/`、`composables/`、`stores/`、`api/` 各放什么。
...
HTTP 请求
   │
   ▼
controller/OrganizationController     ← 收请求、校参数（在 demo-basic-biz 模块）
   │
   ▼
service/OrganizationService           ← 业务逻辑、组合数据（在 demo-basic-service 模块）
   │
   ▼
mapper/OrganizationMapper (+ XML SQL) ← 拼/执行 SQL
   │
   ▼
MySQL                            ← 真正的数据
```

> 示例说明：`OrganizationController` 位于 `demo-basic-biz/src/main/java/com/example/platform/basic/biz/controller/OrganizationController.java`，正是第 04 课用到的 `getById` 接口的所在地。而 service、mapper、entity 则分布在 `demo-basic-service` 模块。这印证了上一节说的——`biz` 是门面层、`service` 是干活层。

**一个关键认知**：包名（package）就是目录路径。`com.example.platform.basic.service.entity` 这个包，对应的就是 `src/main/java/com/example/platform/basic/service/entity/` 这个文件夹。Java 强制两者一致，不像前端可以随意摆放再用相对路径 import。
...
- **多模块 = Java 版 monorepo**：demo-basic 父 pom 用 `packaging=pom` + `<modules>` 聚合 common/service/client/biz 四个子模块，`biz` 是带启动类的门面层，`service` 是业务实现层。
...
- **匿名化示例代码出处**：本课全部取自 `demo-basic` 项目——父 `pom.xml`、`demo-basic-biz/DemoBasicAdminApplication.java`、`bootstrap.yml`、`controller/OrganizationController.java` 及 `demo-basic-service` 的目录结构。

**下一课预告**：第 11 课我们正式动手写代码——从零创建一个 Controller，定义 REST 接口，让你的第一个 HTTP 接口真正跑起来并能用浏览器/Postman 访问。这一课搭好的骨架，下一课就要往里填血肉了。

# 十、总结

- **先建立一个总览类比**：你在前端创建一个项目，脑子里有一张固定的图：package.json 管依赖、vite.config.ts 管构建配置、src/ 放源码、main.ts 是入口。
- **Maven 项目骨架：约定优于配置**：这是 demo-basic 项目里负责"基础数据服务"的模块。
...
- **配置文件：bootstrap.yml / application.yml**：demo-basic 这里只有 bootstrap.yml，因为它把大部分配置放到了远程配置中心 Apollo（类似一个集中管理所有服务配置的后台）。
- **启动类：程序的 main.ts**：每个 Spring Boot 应用有且只有一个启动类，它就是 src/main.ts。

...
> 读完后，你应能完成以下任务：
> - 绘制“Java（15） - MyBatis 与 MyBatis-Plus 入门 / 先回忆：第 04 课的"五站"里，数据库是最后一站”的关键对象与数据流，解释“Controller 收请求、Service 写业务、最后真正"和数据库说话"的是 Mapper。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（15） - MyBatis 与 MyBatis-Plus 入门 / 前端类比：你早就用过"ORM"了”设计正常与异常输入，验证“所以你已经有直觉了：MyBatis 偏"手写 SQL 派"，MyBatis-Plus 偏"自动生成派"。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（15） - MyBatis 与 MyBatis-Plus 入门 / MyBatis 核心三件套：Mapper 接口、XML、动态 SQL”的最小代码或配置，检验“这是 Java 新手最容易懵的点：Mapper 是个 interface，没有实现类，但它能直接被调用并真的查到数据。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先回忆：第 04 课的"五站"里，数据库是最后一站**：Controller 收请求、Service 写业务、最后真正"和数据库说话"的是 Mapper。
- **MyBatis 核心三件套：Mapper 接口、XML、动态 SQL**：这是 Java 新手最容易懵的点：Mapper 是个 interface，没有实现类，但它能直接被调用并真的查到数据。
- **MyBatis-Plus：把 CRUD 省到一行**：" MyBatis-Plus（简称 MP）就是来治这个的。
- **串起来：一次查询的完整数据流**：把本课的零件拼回第 04 课的"五站"图里：
- **本课小结**：Mapper 是接口、没有实现类：MyBatis 用反射 + 动态代理（第 08 课）在运行时生成实现，类比 GraphQL Codegen / Prisma 自动生成的客户端。
- **前端类比：你早就用过"ORM"了**：所以你已经有直觉了：MyBatis 偏"手写 SQL 派"，MyBatis-Plus 偏"自动生成派"。

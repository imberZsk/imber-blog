...
> 读完后，你应能完成以下任务：
> - 绘制“Java（27） - Elasticsearch 全文检索 / 先问一个问题：为什么不用 MySQL 搜？”的关键对象与数据流，解释“LIKE '%xxx%' 在 MySQL 里用不上 B+ 树索引（索引只能加速「前缀匹配」LIKE 'xxx%'），所以本质就是全表逐行扫描。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（27） - Elasticsearch 全文检索 / ES 到底是什么？”设计正常与异常输入，验证“它跟你的业务库（MySQL）是两套独立系统，数据通过同步机制保持一致”，输出首个偏差位置与回归测试结果。
> - 实现“Java（27） - Elasticsearch 全文检索 / 核心概念：倒排索引（Inverted Index）”的最小代码或配置，检验“这是 ES 快的根本原因，也是面试必问。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先问一个问题：为什么不用 MySQL 搜？**：LIKE '%xxx%' 在 MySQL 里用不上 B+ 树索引（索引只能加速「前缀匹配」LIKE 'xxx%'），所以本质就是全表逐行扫描。
- **ES 到底是什么？**：它跟你的业务库（MySQL）是两套独立系统，数据通过同步机制保持一致
- **核心概念：倒排索引（Inverted Index）**：这是 ES 快的根本原因，也是面试必问。
- **三个核心名词：index / document / mapping**：| document（文档） | row（行） | 一个 JS 对象 | 一条资产操作记录就是一个 document |
- **demo 实战：订单与车务的 ES 搜索**：租户隔离写在 query 里：group_id 强制注入，前端传什么都改不了——后端兜底的安全设计，永远不要相信前端传的范围。 -> Map 接 ES 结果：ES 文档是动态的，用 Map 最灵活，再手动转成 DTO（强类型）给上层用。 -> 分页 do-while 循环：导出场景要拉全量，所以一页页捞直到空。
- **一个完整查询长什么样？**：把前面的概念串起来，一个发给 ES 的查询 JSON（概念示意）：

...
> 读完后，你应能完成以下任务：
> - 绘制“Java（19） - 读懂一个完整接口 / 先回到第 4 课埋的那个伏笔”的关键对象与数据流，解释“答案是：同一份"公司数据"，在不同的层有不同的身份，Java 用三种不同的类来表达这三种身份。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（19） - 读懂一个完整接口 / 三种 DTO：In / Out / Entity”设计正常与异常输入，验证“Java 只是把这套"分类型"的纪律，从"可选的好习惯"变成了"强制的工程结构"——每种身份都是一个独立的 .java 文件，放在不同的包里。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（19） - 读懂一个完整接口 / 逐个拆：从数据库那一端往前端方向看”的最小代码或配置，检验“它没有 MyBatis 注解（没有 @TableName、没有 @TableId、不继承 Model）。 -> 它的字段可以和 Entity 不完全一样。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先回到第 4 课埋的那个伏笔**：那时候你可能会想：前端不就是 axios.get(...) 拿到一个 JSON 对象，然后整条链路都用这一个对象吗？
- **三种 DTO：In / Out / Entity**：Java 只是把这套"分类型"的纪律，从"可选的好习惯"变成了"强制的工程结构"——每种身份都是一个独立的 .java 文件，放在不同的包里。
- **逐个拆：从数据库那一端往前端方向看**：它没有 MyBatis 注解（没有 @TableName、没有 @TableId、不继承 Model）。 -> 它的字段可以和 Entity 不完全一样。
- **串起来：getById 这条链路完整走一遍**：这是 demo 里 GET /organization/getById?id=123 的完整代码。
- **灵魂拷问：为什么不直接返回 Entity？**：而 Out 是一个你完全可控的纯数据类：它放哪些字段由接口契约决定。
- **关于 BeanUtils.copyProperties 的几句实话**：| 浅拷贝 | 拷的是引用，嵌套对象不会深拷贝，改目标可能影响源 |

...
> 读完后，你应能完成以下任务：
> - 绘制“Java（24） - Apollo 配置中心 / 为什么不把配置写死在代码里”的关键对象与数据流，解释“Java 后端的 application.yml、application-prod.yml 就是同一个思路（见第 1 课提到的 profiles）。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（24） - Apollo 配置中心 / Apollo 是什么：一句话与一张图”设计正常与异常输入，验证“Apollo（阿波罗）是携程开源的分布式配置中心。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（24） - Apollo 配置中心 / demo 里怎么接入 Apollo（匿名化示例代码）”的最小代码或配置，检验“因为 Apollo 配置必须在应用其余配置加载之前就拿到（比如数据库连接串都在 Apollo 上，没它连不上库）。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **为什么不把配置写死在代码里**：Java 后端的 application.yml、application-prod.yml 就是同一个思路（见第 1 课提到的 profiles）。
- **Apollo 是什么：一句话与一张图**：Apollo（阿波罗）是携程开源的分布式配置中心。
- **demo 里怎么接入 Apollo（匿名化示例代码）**：为什么是 bootstrap.yml 而不是 application.yml？
- **环境隔离：test / staging / prod**：meta 是 Apollo 的「元服务地址」——应用先问它「我的配置在哪台机器上」，再去拉取。
- **在代码里读取配置：两种姿势**：适合零散的、单个的配置值。
- **动态刷新：Apollo 最香的能力**：到这里都还只是「启动时读一次」。

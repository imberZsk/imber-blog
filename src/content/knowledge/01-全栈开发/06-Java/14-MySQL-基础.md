...
> 读完后，你应能完成以下任务：
> - 绘制“Java（14） - MySQL 基础 / 为什么前端工程师必须懂 SQL”的关键对象与数据流，解释“做前端时，你拿到的数据是后端"喂"给你的 JSON。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（14） - MySQL 基础 / 先认识一张真实的表：demo 的 organization 网点表”设计正常与异常输入，验证“【前端类比】CREATE TABLE 就像你用 TypeScript 定义一个 interface Organization { id: string; organizationName: string; ... }，再规定"这个数组里每个元素都必须长这样"。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（14） - MySQL 基础 / CRUD：增删改查四板斧”的最小代码或配置，检验“这是你 90% 时间在写的东西。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先认识一张真实的表：demo 的 organization 网点表**：【前端类比】CREATE TABLE 就像你用 TypeScript 定义一个 interface Organization { id: string; organizationName: string; ... }，再规定"这个数组里每个元素都必须长这样"。
- **CRUD：增删改查四板斧**：这是你 90% 时间在写的东西。
- **WHERE / ORDER BY / LIMIT —— 查询的三大修饰**：| LIKE '北京%' | 模糊匹配（%是通配符） | str.startsWith('北京') |
- **动态拼 SQL：看 demo 匿名化示例代码**：前面的 SQL 都是写死的。
- **JOIN —— 多表关联查询**：这是 SQL 真正强大、也是前端最陌生的地方。

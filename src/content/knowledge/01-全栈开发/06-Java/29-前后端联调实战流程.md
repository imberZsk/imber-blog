...
> 读完后，你应能完成以下任务：
> - 绘制“Java（29） - 前后端联调实战流程 / 先建立全局心智模型”的关键对象与数据流，解释“联调的本质，是让「跑在你电脑上的前端」和「某个地方的后端」对上话。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Java（29） - 前后端联调实战流程 / 本地启动后端服务（组合 B 的前提）”设计正常与异常输入，验证“区别是前端入口挂载组件到 DOM，Java 入口是把整个 Spring 容器拉起来、扫描所有 @RestController、开始监听端口。”，输出首个偏差位置与回归测试结果。
> - 实现“Java（29） - 前后端联调实战流程 / demo-basic 的启动入口”的最小代码或配置，检验“spring.profiles.active: dev ≈ 前端的 NODE_ENV=development，决定加载哪一套环境变量。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **本地启动后端服务（组合 B 的前提）**：| npm run dev 启动 dev-server | 在 IDEA 里运行 XxxApplication.java 的 main 方法 | 都是起一个常驻进程监听端口 |
- **前端代理配置（proxy 指向后端）**：这是你的主场，但 demo 用 webpack-dev-server，配置位置和 Vite 略有不同。
- **看接口文档：先搞清楚要调什么**：联调第一步永远是「读接口文档」。
- **用 Postman / Apifox 单独测接口**：【前端类比】等价于你绕过前端、直接用 Postman 验证后端契约，跟你以前调第三方 API 一个套路。
- **查看后端日志定位问题**：学会看日志是后端联调的核心技能。

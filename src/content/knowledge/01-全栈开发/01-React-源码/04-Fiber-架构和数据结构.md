# React 源码（04） - Fiber 架构和数据结构

> 读完后，你应能完成以下任务：
> - 绘制“React 源码（04） - Fiber 架构和数据结构 / fiber 的三种含义”的关键对象与数据流，解释“先来看 FiberNode 这个构造函数，这个构造函数源码中有三个注释，翻译过来就是 实例，Fiber，副作用，再参考 React 技术中的理念”，并用源码位置、日志或 Trace 标注证据。
> - 为“React 源码（04） - Fiber 架构和数据结构 / 状态和属性”设计正常与异常输入，验证“React 通过比较这两个属性来判断组件的 props 是否发生变化，从而决定是否需要重新渲染。”，输出首个偏差位置与回归测试结果。
> - 实现“React 源码（04） - Fiber 架构和数据结构 / 工作标签系统（workTag）”的最小代码或配置，检验“React Fiber 的“工作标签系统”主要是指 Fiber 节点上的 tag 属性。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **状态和属性**：React 通过比较这两个属性来判断组件的 props 是否发生变化，从而决定是否需要重新渲染。
- **工作标签系统（workTag）**：React Fiber 的“工作标签系统”主要是指 Fiber 节点上的 tag 属性。
- **fiber 的指针结构**：child: 从父节点指向其第一个子节点。
- **Fiber 树的构建与遍历**：Fiber 树的构建和遍历是协调阶段的核心。
- **双缓冲 Fiber 树？**：current 树 (当前树): 这棵树代表了当前已经渲染到屏幕上的 UI 状态。 -> workInProgress 树 (工作中的树): 这棵树是 React 在后台构建或更新的树。 -> 原子性更新与一致性: React 可以在 workInProgress 树上完成所有的计算和准备工作，而不会影响当前屏幕上显示的 UI。 -> 可中断与恢复渲染: 由于所有的工作都在 workInProgress 树上进行，如果渲染过程中有更高优先级的任务（如用户输入事件）到来，React 可以暂停 workInProgress 树的构建，去处理高优先级任务，然后再回来从之前中断的地方继续构建 workInProgress 树。

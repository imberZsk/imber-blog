# React 源码（06） - Lane 车道模型

> 读完后，你应能完成以下任务：
> - 绘制“React 源码（06） - Lane 车道模型 / 车道模型”的关键对象与数据流，解释“为了保证交通顺畅，系统必须智能地为不同类型的车辆分配专用车道，并规定它们的通行优先级。”，并用源码位置、日志或 Trace 标注证据。
> - 为“React 源码（06） - Lane 车道模型 / 位掩码”设计正常与异常输入，验证“要实现如此高效灵活的“车道”管理系统，React 选择了位掩码（Bitmask）作为其技术基石。”，输出首个偏差位置与回归测试结果。
> - 实现“React 源码（06） - Lane 车道模型 / Lane 的类型分类”的最小代码或配置，检验“SyncLane: 最高优先级，用于必须同步执行的更新（例如，由 flushSync 触发的更新，或某些离散的用户输入）。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **车道模型**：为了保证交通顺畅，系统必须智能地为不同类型的车辆分配专用车道，并规定它们的通行优先级。
- **位掩码**：要实现如此高效灵活的“车道”管理系统，React 选择了位掩码（Bitmask）作为其技术基石。
- **Lane 的类型分类**：SyncLane: 最高优先级，用于必须同步执行的更新（例如，由 flushSync 触发的更新，或某些离散的用户输入）。
- **如何分配一个合适的 Lane**：这个分配过程并不是随机的，而是会综合考虑当前 React 应用的多种运行情况和上下文信息，以确保更新能够以恰当的优先级和方式进行处理。
- **多车道协同作战：Lanes 的合并与选择**：我们已经了解了 React 如何为单次更新分配一个合适的 Lane，但这只是故事的一部分。
- **案例：调度策略的实际体现**：初始状态: 组件加载，useEffect 中的 setTimeout 触发 setList。 -> 调度决策: 调度器调用 getNextLanes，发现 pendingLanes 中只有 DefaultLane，于是返回 DefaultLane，React 开始进行列表的渲染工作。 -> 中断发生: 在列表渲染的过程中，用户在输入框打字，触发 onChange。 -> 新的调度决策: 由于新的高优先级任务到来，React 中断当前渲染。

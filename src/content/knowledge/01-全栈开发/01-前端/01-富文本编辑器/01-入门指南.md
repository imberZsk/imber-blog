...
Web 富文本编辑器的发展史，正是一个从严重依赖浏览器原生 API（如 document.execCommand），到逐步构建更独立、更可控的编辑模型，甚至最终“抛弃”浏览器原生编辑能力的过程。
...
**代表产品：** Quill（打不开）, Draft.js（不更新了）, Slate（路线陡峭）, Prosemirror,
CKEditor 5，lexical（新还不稳定）

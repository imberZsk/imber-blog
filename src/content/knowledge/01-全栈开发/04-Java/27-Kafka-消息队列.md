...
> 读完后，你应能解释“1.1 MQ 的三大价值”，复现“2.1 Topic：消息的"频道"”的最小实现，并用“2.2 Partition：Topic 的并行分片（Kafka 性能的关键）”检查结果与失败边界。
...
- **Kafka 的四个核心概念**：先建立一张和前端对照的总表，后面逐个拆解：
- **为什么需要 MQ？从一个前端类比说起**：在前端，假设"用户下单成功"后要做三件事：弹 toast、刷新购物车角标、上报埋点。
- **避坑提示（前端思维容易踩的坑）**：消息可能重复消费：网络抖动、消费者重启都可能导致同一条消息被处理两次。

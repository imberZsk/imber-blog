...
## 一、先建立前端锚点：tool use ≈ 给模型一张「函数注册表」
...
// 来了个命令名，就去表里找对应函数来调
const result = await handlers[cmd.name](...cmd.args)
```

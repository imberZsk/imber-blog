# Python（38） - A3 - 附录-JS到Python速查表

> 读完你能：围绕“A3 - 附录-JS到Python速查表”理解“怎么用本篇”与“变量与基本类型”，并结合正文示例完成实践与排障。
...
## 四、高阶函数：map / filter / reduce → 列表推导式
...
### 16.2 Pandas DataFrame ≈ 加强版 Excel / SQL
...
### 16.3 matplotlib ≈ 后端版 ECharts / D3
...
### 17.4 Agent / 工具调用 ≈ 带记忆的 while 循环 + 函数注册表
...
✅ **该掌握（核心迁移直觉）**
- Python ≈ 不写类型的 JS + 缩进代替 `{}` + 换行代替 `;`，上手比 Java 顺。
- 高阶操作优先用**列表推导式** `[x for x in arr if ...]`，别一股脑搬 map/filter。
- **切片** `arr[1:3]` / `arr[::-1]` 是 Python 杀手锏，list/str/tuple 通用。
- async/await、装饰器 `@`、生成器 `yield` 都和 JS/TS 一对一映射，能直接搬写法。
- 阶段四五的库认准锚点：NumPy ≈ 超级 Array，DataFrame ≈ Excel/SQL，matplotlib ≈ 后端 ECharts；Embedding ≈ 把文字压成坐标，RAG ≈ 开卷考试，Agent ≈ 带记忆的 while 循环 + 函数注册表。

⚠️ **易混淆（看到必须停一下）**
- `True` / `False` / `None` 首字母大写，写小写直接报错。
- `is` ≠ `===`：**只用于 `is None`**，其余比较一律 `==`。
- `self` ≠ `this`：必须手写为方法第一个参数，不会自动注入。
- 可变默认参数 `def f(x=[])` 会跨调用共享，默认值用 `None` 再函数内新建。
- dict 取值用方括号 `d["k"]` 或 `.get("k", 默认)`，**不能用点号** `d.k`。
- `arr.join(",")` 在 Python 是 `",".join(arr)`，主客颠倒。

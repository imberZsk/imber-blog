...
> 读完后，你应能解释“1.1 为什么不直接把整个知识库塞进 prompt？”，复现“4.1 离线：把片段灌进库”的最小实现，并用“4.2 在线：用余弦相似度找 top-k”检查结果与失败边界。
...
RAG 就是给大模型办了张「开卷考试」准考证：用户提问时，先去你的资料库**检索**出最相关的几段，再把它们**拼进 prompt** 让模型**照着资料生成答案**。它不改模型、不微调，纯靠「检索 + 上下文注入」就让通用模型答得上你的私有问题，是当前落地最广的 AI 应用形态。整条流水线 = 离线建索引（切分 → embedding → 入向量库）+ 在线问答（问题 embedding → top-k 检索 → 拼 prompt → 生成），你熟悉的「搜索 → 拿数据 → 渲染」心智模型可以直接平移。

✅ **该掌握**
- RAG 三步走：**检索（retrieval）→ 拼装（augment）→ 生成（generation）**，对应离线建索引 + 在线问答两条线。
- **chunking** 是地基：长文档必须切成小片段再做 embedding，常用「固定长度 + overlap」。
- 检索用**余弦相似度取 top-k**（复用第 25 篇）；小规模可 numpy 手算，规模化上 Chroma / FAISS / pgvector 等向量库。
- 生成靠 **system prompt 立规矩**：「只依据资料回答，找不到就说不知道」，这是防幻觉的核心护栏。
- 一个能跑的最小 RAG 不到 50 行，是文档问答 / 智能客服的通用内核。

⚠️ **易混淆（前端最容易栽的坑）**
- **RAG ≠ 微调**：RAG 只在 prompt 里塞资料，不改模型权重。
- **建库与查询的 embedding 模型必须一致**，否则向量坐标系不通用，检索全乱。
- **检索是语义匹配，不是关键词匹配**：字面不重合也能命中。
- **不能把整个知识库塞进 prompt**：窗口装不下、贵、噪声还拖垮效果——所以才要先检索。
- **RAG 的天花板是检索不是生成**：答不准先排查「召回的片段对不对」，把它打印出来肉眼看，别急着换模型。

下一篇（第 27 篇）：把「检索」升级成「让模型自己决定调哪个工具」——Agent 与 tool use，ReAct = 带记忆的「思考 → 调工具 → 再思考」循环。

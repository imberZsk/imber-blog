# 工程化脚手架（02） - 项目打包与发布 NPM

> 读完后，你应能完成以下任务：
> - 绘制“工程化脚手架（02） - 项目打包与发布 NPM / 概述”的关键对象与数据流，解释“Imber CLI 的打包与发布系统是一个完整的 NPM 包管理解决方案，支持 monorepo 架构下的多包发布、版本管理、依赖优化和自动化部署。”，并用源码位置、日志或 Trace 标注证据。
> - 为“工程化脚手架（02） - 项目打包与发布 NPM / 打包系统架构”设计正常与异常输入，验证“构建系统：TypeScript 编译、代码优化、资源处理 -> 包管理器：依赖分析、包结构生成、元数据管理 -> 版本控制：语义化版本、变更记录、发布流程 -> 发布系统：NPM 发布、CDN 同步、文档生成”，输出首个偏差位置与回归测试结果。
> - 实现“工程化脚手架（02） - 项目打包与发布 NPM / 构建系统实现”的最小代码或配置，检验“构建的输入契约是源码、锁文件和构建配置，输出契约是可被目标 Node.js 或浏览器加载的 JavaScript、类型声明、Source Map 与必要资源。”，输出命令、结果与 Diff，并说明不适用边界。
...
构建的输入契约是源码、锁文件和构建配置，输出契约是可被目标 Node.js 或浏览器加载的 JavaScript、类型声明、Source Map 与必要资源。构建成功不能只看命令退出码，还要在临时目录执行 `npm pack --dry-run`，确认发布包没有带入源码密钥、测试夹具和本地配置，也没有漏掉运行时文件。

Monorepo 发布必须先按依赖图构建底层包，再构建引用它们的上层包；仅按目录名排序会在缓存为空的 CI 中暴露未生成声明文件的问题。包之间应通过 `exports` 暴露稳定入口，消费者不应依赖 `dist` 内部路径，否则重排产物目录会成为破坏性变更。

发布物需要同时记录 Git commit、包版本、构建环境和制品摘要。回滚不是覆盖同一个 NPM 版本，而是撤销错误的 `dist-tag` 或发布一个修复版本；NPM 版本一旦被消费就应视为不可变制品。
...
- **构建契约**：输入是源码、锁文件和构建配置，输出必须同时覆盖运行代码、类型声明、Source Map 与必要资源。
- **发布前检查**：使用 `npm pack --dry-run` 审查实际制品，避免漏文件，也避免把密钥、测试夹具和本地配置带入包。
- **Monorepo 顺序**：按包依赖图从底层到上层构建，不能依赖本地缓存或目录名排序掩盖声明文件缺失。
- **回滚边界**：已发布版本保持不可变，通过调整 `dist-tag` 或发布修复版本恢复，并保留 commit、环境和制品摘要。

# 富文本编辑器（03） - 菜单栏和 slash command

> 读完后，你应能完成以下任务：
> - 绘制“富文本编辑器（03） - 菜单栏和 slash command / 前言”的关键对象与数据流，解释“TipTap 编辑器 Notion 风格的时候，需要选中文本提供菜单栏，而不是固定在页面，是动态的，然后支持输入 / 唤醒垂直菜单栏，这些都是 simple tiptap 没有的，需要自己实现。”，并用源码位置、日志或 Trace 标注证据。
> - 为“富文本编辑器（03） - 菜单栏和 slash command / 基础 Demo”设计正常与异常输入，验证“编辑器组件只负责创建 Editor 实例、注册扩展和承载正文，菜单状态交给 MenuBar。”，输出首个偏差位置与回归测试结果。
> - 实现“富文本编辑器（03） - 菜单栏和 slash command / 基础按钮组件”的最小代码或配置，检验“按钮是否高亮来自 editor.isActive()，能否执行应由命令的 can() 结果决定；”，输出命令、结果与 Diff，并说明不适用边界。
...
- **基础 Demo**：编辑器组件只负责创建 Editor 实例、注册扩展和承载正文，菜单状态交给 MenuBar。
- **slash command**：ProseMirror 指南
- **抽离组件**：编辑器组件只负责创建 Editor 实例、注册扩展和承载正文，菜单状态交给 MenuBar。
- **基础按钮组件**：按钮是否高亮来自 editor.isActive()，能否执行应由命令的 can() 结果决定；
- **菜单栏组件**：菜单栏通过 editor.chain().focus().toggleXxx().run() 串起“恢复选区焦点、切换 Mark、提交事务”。

...
> 读完后，你应能完成以下任务：
> - 绘制“富文本编辑器（02） - 样式隔离 / 前言”的关键对象与数据流，解释“在开发富文本编辑器时，样式隔离是一个很关键的问题。”，并用源码位置、日志或 Trace 标注证据。
> - 为“富文本编辑器（02） - 样式隔离 / 样式隔离面临的挑战”设计正常与异常输入，验证“页面的 h1、h2 等标题样式可能影响编辑器内的标题显示”，输出首个偏差位置与回归测试结果。
> - 实现“富文本编辑器（02） - 样式隔离 / 编辑器样式泄露”的最小代码或配置，检验“编辑器的字体、颜色设置可能影响页面布局”，输出命令、结果与 Diff，并说明不适用边界。
...
在开发富文本编辑器时，样式隔离是一个很关键的问题。编辑器的样式需要与页面其他部分的样式完全隔离，避免相互影响。同时，编辑器内部的各种组件（如标题、段落、列表等）也需要有统一的样式管理方案。

这篇我们来讲讲如何基于 Tailwind
...
- **前言**：在开发富文本编辑器时，样式隔离是一个很关键的问题。
- **基于 Tailwind 的样式隔离方案**：完全无 CSS 方案 - 所有样式都通过 Tailwind 类名实现，避免自定义 CSS 文件 -> 作用域隔离 - 使用 Tailwind 的作用域前缀确保样式不会泄露 -> 主题一致性 - 编辑器主题与页面主题保持同步 -> 可扩展性 - 支持多种主题和自定义样式
- **prosemirror-devtools**：然后按谷歌插件，prosemirror-devtools，来依次写样式，这样可以知道编辑器已经有哪些扩展，也避免某些扩展没有写到样式，对于不需要样式的我们也写个空串，cn 函数会帮我们优化类名问题
- **工程边界**：编辑器的样式需要与页面其他部分的样式完全隔离，避免相互影响。
- **实现机制**：完全无 CSS 方案 - 所有样式都通过 Tailwind 类名实现，避免自定义 CSS 文件
- **选型依据**：区别于官网的样式方案：https://tiptap.dev/docs/editor/getting-started/style-editor（sass +

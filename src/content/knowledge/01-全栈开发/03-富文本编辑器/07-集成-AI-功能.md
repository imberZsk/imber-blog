...
> 读完后，你应能完成以下任务：
> - 绘制“富文本编辑器（07） - 集成 AI 功能 / AI 辅助编辑功能”的关键对象与数据流，解释“扩展内容 - 丰富和详细化文本 -> 改进文本 - 优化表达和语法 -> 总结内容 - 生成简洁摘要 -> 翻译文本 - 多语言翻译支持”，并用源码位置、日志或 Trace 标注证据。
> - 为“富文本编辑器（07） - 集成 AI 功能 / 功能特性”设计正常与异常输入，验证“🤖 智能文本处理：支持扩展、改进、总结、翻译等 6 种 AI 功能”，输出首个偏差位置与回归测试结果。
> - 实现“富文本编辑器（07） - 集成 AI 功能 / 流式响应处理”的最小代码或配置，检验“累积内容：智能管理内容更新，避免重复插入”，输出命令、结果与 Diff，并说明不适用边界。
...
- **AI 辅助编辑功能**：扩展内容 - 丰富和详细化文本 -> 改进文本 - 优化表达和语法 -> 总结内容 - 生成简洁摘要 -> 翻译文本 - 多语言翻译支持
- **功能特性**：🤖 智能文本处理：支持扩展、改进、总结、翻译等 6 种 AI 功能
- **流式响应处理**：累积内容：智能管理内容更新，避免重复插入
- **智能文本选择**：选择检测：仅在文本选中时显示 AI 按钮
- **用户体验优化**：错误恢复：AI 失败时自动恢复原始文本
- **配置灵活性**：多服务支持：支持 OpenAI、通义千问等多种 AI 服务

# Playwright（03） - UI 模式完全指南：交互式调试工作流

> 读完后，你应能解释“3.1 一句话理解”，复现“3.2 它解决了什么痛点”的最小实现，并用“3.3 什么时候该用 UI Mode”检查结果与失败边界。
...
UI Mode 只用于本地交互式调试；CI 上用 Trace Viewer 复盘失败。
...
1. UI Mode 负责本地交互式调试和自动重跑。
2. Time Travel 快照、Pick Locator 和 Trace Viewer 分别用于还原现场、校验定位器和复盘 CI 失败。
3. CI 配置 `on-first-retry` 录制 trace，失败后下载并回放。

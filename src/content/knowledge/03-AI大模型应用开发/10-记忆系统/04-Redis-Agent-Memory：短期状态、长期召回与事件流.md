# 记忆系统（04） - Redis Agent Memory：短期状态、长期召回与事件流

> 读完后，你应能完成以下任务：
> - 绘制“记忆系统（04） - Redis Agent Memory：短期状态、长期召回与事件流 / 先按访问模式选数据结构”的关键对象与数据流，解释“Redis 不只是“把 messages 放进 List”。”，并用源码位置、日志或 Trace 标注证据。
> - 为“记忆系统（04） - Redis Agent Memory：短期状态、长期召回与事件流 / 短期会话的安全实现”设计正常与异常输入，验证“滑动 TTL 的含义是：只有通过身份和线程所有权校验的合法访问才刷新过期时间。”，输出首个偏差位置与回归测试结果。
> - 实现“记忆系统（04） - Redis Agent Memory：短期状态、长期召回与事件流 / 事件流与异步总结”的最小代码或配置，检验“总结器使用 Consumer Group 消费事件并写回滚动摘要。”，输出命令、结果与 Diff，并说明不适用边界。

> 更新日期：2026/08/11

本文是 AI 应用开发中 **Redis 短期记忆的主文**。Redis 在 RAG 中还可承担检索缓存和限流，但缓存键、失效维度与验收指标不同，详见 [RAG（08） - Redis 检索缓存、语义缓存与限流](/knowledge/03-AI大模型应用开发/06-RAG/08-Redis-检索缓存、语义缓存与限流)。不要把会话窗口与 RAG 候选缓存放进同一套键空间、TTL 和淘汰策略。
...
滑动 TTL 的含义是：只有通过身份和线程所有权校验的合法访问才刷新过期时间。写入工作状态、追加最近消息和读取会话都要使用同一条原子更新边界；无效请求不能续期，否则攻击流量会让本应删除的会话永久存活。
...
- **先按访问模式选数据结构**：Redis 不只是“把 messages 放进 List”。
- **短期会话的安全实现**：键必须包含租户、用户和线程。
- **事件流与异步总结**：总结器使用 Consumer Group 消费事件并写回滚动摘要。
- **长期记忆也可以放 Redis，但不是同一个键**：写入前可用同一索引查近邻，避免重复记忆；
- **缓存、锁与幂等**：生成缓存键至少包含模型、Prompt 模板版本、知识库版本、权限摘要和规范化问题。
- **验收与故障注入**：Redis 短暂不可用时，聊天是降级为无记忆，还是明确失败。 -> 同一线程两个请求并发更新时，是否丢消息或覆盖新摘要。 -> TTL 到期后，主库中的业务事实是否仍可恢复。 -> 跨租户、跨用户读取是否始终返回空。
...
# 九、动手实践：Redis Session Memory 机制
...
## 9.3 可运行源码：Redis Agent Memory：短期状态、长期召回与事件流

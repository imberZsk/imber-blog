...
而上面说过，每多聊一轮，整个历史都要重发一遍。所以**输入 token 是随轮数累加的**。demo 里场景 1 输入 26 token，到场景 2 多聊一轮就涨到 73——这不是个小事，长会话不管理上下文，账单和延迟都会失控。这也是Prompt 工程（05）《多轮对话与上下文管理》要专门解决的问题。
...
- **把整段历史无脑全发**，会话一长就触发上下文超限报错或账单暴涨。要做截断或摘要（Prompt 工程（05）《多轮对话与上下文管理》）。

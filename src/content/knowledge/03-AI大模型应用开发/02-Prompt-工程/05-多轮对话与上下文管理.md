...
模型要答对，就得知道前面聊了啥。但大模型基础（06）《大模型 API 基础》说过，模型**没有记忆**，每轮都得把历史重发一遍。于是问题来了：20 轮历史全发，token 爆了（触发上下文长度上限直接报错），而且越发越贵越慢；只发最近 2 轮，模型又忘了「这个」是啥。

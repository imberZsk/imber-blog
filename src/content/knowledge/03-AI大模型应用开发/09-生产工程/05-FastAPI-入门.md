...
- **AI 接口照搬普通接口的超时**。模型调用可能要好几秒，默认超时（有的网关 30 秒甚至更短）容易把长回答掐断。AI 接口的超时要单独放宽，长回答更该上流式（生产工程（04）《流式响应》）。
...
- 把 `answer_question` 换成Prompt 工程（05）《多轮对话与上下文管理》的多轮对话逻辑，让接口支持 sessionId。

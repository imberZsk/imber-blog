# Tool 与 Function Calling（05） - Tool 执行安全：权限、确认、幂等与审计

> 读完后，你应能完成以下任务：
> - 绘制“Tool 与 Function Calling（05） - Tool 执行安全：权限、确认、幂等与审计 / 防线一：拦 Prompt 注入”的关键对象与数据流，解释“Prompt 注入是指用户用输入「劫持」模型，让它无视你设定的规则。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Tool 与 Function Calling（05） - Tool 执行安全：权限、确认、幂等与审计 / 防线二：拦密钥与提示词泄露”设计正常与异常输入，验证“只要输入里出现这类敏感词就拦——「api key」「密钥」「system prompt」这些词，”，输出首个偏差位置与回归测试结果。
> - 实现“Tool 与 Function Calling（05） - Tool 执行安全：权限、确认、幂等与审计 / 防线三：拦越权工具调用”的最小代码或配置，检验“这是 Agent 特有、也最危险的一类。”，输出命令、结果与 Diff，并说明不适用边界。
...
Chatbot 顶多被诱导说几句不该说的话。
但 Agent 能调工具、能动真实数据，
一旦被攻破，
损失是实打实的——删了数据、泄了密钥、越权操作了别人的账户。
所以 Agent 的安全边界，比纯聊天严肃得多。

核心原则一句话：**安全不能指望模型「自觉」，必须由代码硬拦。
** 模型是可以被语言诱导的，
你写在 prompt 里的「请不要做 X」，
攻击者用几句话就能绕过。
真正的护栏只能是代码里的硬校验。
...
再次强调：权限判断绝不能写进 prompt 让模型自己把关，只能是后端的硬校验。
同一个「删库」请求，
有权限的放行、没权限的拦掉，
区别完全在这段代码，
和模型说什么无关（这正是 28 篇的核心，
安全在后端校验，
不在模型）。
...
它只能是后端的硬校验。
规则表（正则）只能挡住已知套路，
真实项目还要配合输出侧检查（回答里不能带密钥）、工具调用审计日志、写操作人工确认（见 Tool 与 Function Calling（01）《Function Calling 与 Tool Calling》）。
安全是分层的，没有一招通吃。

## 7.5 可运行源码：Agent 安全边界
...
- **防线二：拦密钥与提示词泄露**：只要输入里出现这类敏感词就拦——「api key」「密钥」「system prompt」这些词，
- **防线三：拦越权工具调用**：这是 Agent 特有、也最危险的一类。
- **工程上真正会踩的坑（本篇独有）**：把安全规则写进 prompt 指望模型自觉。
- **一句话面试答法**：核心原则是安全靠代码硬拦，不靠模型自觉——模型能被语言诱导，prompt 里的「请不要」没用。

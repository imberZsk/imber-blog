...
Karpathy 提出的 LLM Wiki 把一部分工作前移到摄取阶段：新资料进入后，
LLM 不只建立索引，
还更新主题页、实体页、对比页、交叉链接与冲突说明。
下次查询先读已经整理过的 Wiki，复用持续积累的综合结果。
这里的“编译”是工程类比，不表示模型生成内容天然正确。
...
读图时重点看两条闭环：新来源经 Ingest 更新持久 Wiki，
高价值 Query 与 Lint 发现的缺口又回到维护流程；
Schema 则持续约束每次写入的目录、格式和审核规则。

# 二、三层结构与三类操作

**Raw Sources** 是事实依据，如制度原文、会议纪要和论文。

# 项目实战（07） - 五分钟跑起来：本地启动你的控制台

> 读完后，你应能完成以下任务：
> - 绘制“项目实战（07） - 五分钟跑起来：本地启动你的控制台 / 先决条件”的关键对象与数据流，解释“Node.js ≥ 20（必须）”，并用源码位置、日志或 Trace 标注证据。
> - 为“项目实战（07） - 五分钟跑起来：本地启动你的控制台 / 最快路径：一行命令上手”设计正常与异常输入，验证“走一遍引导式配置（--yes 表示全部用默认值，不逐项问你） -> 配好环境、数据目录、密钥 -> 启动 API 与同源 UI，并打开本地控制台”，输出首个偏差位置与回归测试结果。
> - 实现“项目实战（07） - 五分钟跑起来：本地启动你的控制台 / 之后再次启动”的最小代码或配置，检验“⚠️ 重要约定：如果你是用 npx 装的，之后所有命令都要用 npx paperclipai ... 这个前缀。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **先决条件**：Node.js ≥ 20（必须）
- **最快路径：一行命令上手**：走一遍引导式配置（--yes 表示全部用默认值，不逐项问你） -> 配好环境、数据目录、密钥 -> 启动 API 与同源 UI，并打开本地控制台
- **启动链路：命令背后发生了什么**：onboard 负责生成实例配置与本地密钥，doctor 校验配置、数据库、存储和密钥，run 在检查通过后启动 Node.js API；
- **用健康检查验收启动**：| Mode embedded-postgres | 用的是内嵌 PostgreSQL（不是外部数据库） |
- **数据都存在哪？（重要）**：Paperclip 把所有数据放在你 home 目录下的 /.paperclip/，结构是「一个实例（instance）下挂多家公司」。
- **另一条路：从源码跑（开发者向）**：如果你想改 Paperclip 本身的代码、或者要看它的源码学习，就用克隆仓库的方式。

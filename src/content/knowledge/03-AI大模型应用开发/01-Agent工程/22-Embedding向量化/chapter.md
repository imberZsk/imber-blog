...
```bash
cd demos/22-embedding
python3 main.py
```

`main.py` 没有真实 embedding 模型，用最朴素的「词袋」当向量：向量的每一维对应词表里一个词，值是这个词在文本里出现的次数。核心函数：

- `to_vector(text, vocab)` —— 把文本按统一词表转成词频向量；
- `cosine(v1, v2)` —— 算两个向量的余弦相似度；
- `build_vocab` —— 用所有文本建一个统一词表，保证向量维度可比较。

跑完你会看到：基准问题「报销需要几天内提交」，和「报销请在三十天内提交申请」相似度最高，和「附上发票原件」次之，和「今天午餐吃什么」是 0。排序对了，这就是检索「最相关排前面」的底层逻辑。
...
`23-向量数据库.md` —— 文本都转成向量了，几万几百万个向量存哪、怎么快速找出最相近的几个？这就是向量数据库的活。

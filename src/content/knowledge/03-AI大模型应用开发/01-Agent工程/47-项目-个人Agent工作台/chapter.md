...
```bash
cd demos/47-personal-agent-workbench
python3 main.py
```

`main.py` 把一个 4 步工作流跑了三遍，对应三种状态：

- `plan`：把目标拆成 4 个有序步骤（真实项目换成模型规划）。
- `Workbench._resolve_args`：解析 `from_step` 引用，把步骤依赖接起来。
- `execute` 的 `fan_out` 分支：给每个延期任务建提醒。
- `execute` 返回 `paused`：写操作前暂停等确认。
- `_run_tool` 的权限校验：只读用户在建提醒这步被 `blocked`。
- `self.trace`：每一步都记下来，整个任务可观察。

对比场景 1（paused）和场景 2（done）最直观：同样的计划，没确认停在第 2 步，确认了一路跑到日报草稿。

...
```bash
cd demos/23-vector-db
python3 main.py
```

`main.py` 用 `dict + list` 实现了一个最小向量库 `MiniVectorStore`，把上面两个操作讲清楚：

- `MiniVectorStore.add(text, metadata)` —— 入库时就把文本向量化（`embed`），连同原文和元数据存进内部的 `records` 列表；
- `MiniVectorStore.search(query, top_k)` —— 把查询向量化，和库里每条算余弦相似度（`cosine`），返回最相近的 topK。

这就是「暴力检索」的最小版本——逐条比一遍。跑一下你会看到「年会什么时候开」命中活动通知、「年假有几天」命中人事制度，各自带回了元数据里的来源。

...
```bash
cd demos/12-structured-output
python3 main.py
```

`main.py` 构造了 5 种典型模型输出，把三道防线挨个触发一遍：

- `extract_json`：第一关，演示怎么从 ```json 代码块里把 JSON 抠出来。
- `validate` + `SCHEMA`：第二关，「枚举越界」和「缺字段」两个场景都在这里被拦。
- `retry_once` + `FALLBACK`：第三关，最后那条「大白话」场景重试也救不回来，最终走兜底。
- `process`：把三关串成完整流程，对应你后端真实的处理顺序。

跑完你会看到：无论模型怎么乱来，函数返回的永远是字段齐全、枚举合法的 dict。

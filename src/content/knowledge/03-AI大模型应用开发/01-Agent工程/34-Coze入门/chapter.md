...
```bash
cd demos/34-coze-intro
python3 main.py
```

`main.py` 用纯标准库还原了一个挂了天气、股票两个插件的 Bot，跑三个场景：问天气（命中天气插件）、问股价（命中股票插件）、问股价但没给代码（参数缺失被拦）。

核心函数对应关系：
- `PLUGINS` —— Coze 里你给 Bot 勾选的插件，`triggers` 对应插件的功能描述
- `bot_select_plugin` —— Bot 读描述选插件（模型提议）
- `_extract_args` —— 从问题里抽插件参数
- `run_plugin` —— 平台执行插件 + 参数校验
- `bot_reply` —— Bot 用插件结果生成回答

运行时会打印 Bot 的决策过程（选了哪个插件、传了什么、插件返回什么），这条 trace 就是 Coze 后台「调试」面板给你看的东西。

...
```bash
cd demos/35-langchain-intro
python3 main.py
```

**不需要 `pip install langchain`**，纯标准库就跑。`main.py` 把五个积木都实现了，先单独演示每个，最后用 `Chain` 把 Retriever + Prompt + LLM 串成一条 RAG 流水线。

核心对应关系：
- `PromptTemplate` / `FakeLLM` / `SimpleRetriever` / `Tool` —— 四个基础积木
- `Chain.__or__` —— LCEL 的 `|` 串联，重载运算符把组件接成管道
- 最后的 RAG 链 —— 等价于真实 LangChain 的 `retrieve | prompt | llm`

跑通后对照真实 LangChain 的 API 看，你会发现「哦，原来 `ChatOpenAI` 就是我的 `FakeLLM`，`prompt | llm` 就是我的 `Chain.__or__`」。

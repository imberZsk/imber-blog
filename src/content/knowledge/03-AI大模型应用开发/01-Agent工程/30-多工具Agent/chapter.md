...
这样拆不是为了“分角色聊天”，而是为了减少上下文、降低工具混淆、隔离权限。MCP 这类共享服务可以把检索、记忆、Prompt 模板、外部工具统一暴露给不同 Agent，用法见 `../appendices/进阶-多Agent与MCP工程化.md`。

## 配套 demo：跑起来看

```bash
cd demos/30-multi-tool-agent
python3 main.py
```

`main.py` 注册了 3 个工具，核心是 `TOOL_REGISTRY`（注册表）和 `route`（路由）：

- `route(question)` —— 遍历注册表，按关键词命中选工具；
- 每个工具自带 `extract`（抽参数）和 `run`（执行）；
- `run(question)` 里 `if tool is None` 就是无匹配兜底。

跑完看 4 个问题：查订单、算报销、建工单分别路由到对应工具，「今天天气怎么样」没有匹配工具，走兜底。

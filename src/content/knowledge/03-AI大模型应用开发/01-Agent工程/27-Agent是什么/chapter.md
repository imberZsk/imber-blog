...
```bash
cd demos/27-agent-intro
python3 main.py
```

`main.py` 让三种系统处理开头那个同样的问题，把各自的轨迹打出来：

- `run_chatbot` —— 只生成文字，承认查不了订单；
- `run_workflow` —— 固定三步，无条件建工单（注意它「不看是否真有风险」就建）；
- `run_agent` —— 先查订单，`if risky` 才建工单，根据观察动态决策。

对照三段输出，差别一目了然：同一个问题，Workflow 机械地走完固定步骤，Agent 看了查询结果才决定要不要建工单。

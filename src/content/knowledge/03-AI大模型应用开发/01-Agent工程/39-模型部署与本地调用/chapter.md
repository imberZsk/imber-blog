...
```bash
cd demos/39-model-deploy
python3 main.py
```

`main.py` 干了两件事：用标准库 `http.server` 起一个**自己实现的 OpenAI 兼容 mock 模型服务**，再用上面那个通用 `chat` 客户端去调它。一条命令自动起服务、调两次、打印回答和 token 用量、关服务。

核心对应关系：
- `MockModelHandler.do_POST` —— 模型服务端，实现 `/v1/chat/completions`，请求响应结构对齐 OpenAI
- `_fake_infer` —— 模型推理的位置（真实模型在这里做推理）
- `chat` —— 通用客户端，换 `BASE_URL` 即可调真实 OpenAI/Ollama
- 响应里的 `usage` —— token 用量统计，和 OpenAI 字段一致

跑通后你就理解了：你写的这个 mock 服务，和真实 OpenAI 在接口层面没有区别，客户端分不出来。

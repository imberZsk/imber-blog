...
```bash
cd demos/25-answer-citation
python3 main.py
```

`main.py` 把这三件事拆成可见的步骤，核心是 `generate_answer(question, min_score)`：

- 正常路径：`retrieve` 命中后，用命中的 chunk 作回答主体，`citations` 直接取自这些 chunk 的 source；
- 无资料拒答：`if not hits` 分支；
- 弱证据拒答：`hits[0]["score"] < min_score` 分支。

跑完看三条问题：「退货邮费谁出」强命中、正常回答带引用；「邮费贵不贵」只蹭到一个片段、触发弱证据拒答；「门店在哪里」完全没资料、拒答。两种拒答文案不同。
...
`26-RAG评测与调优.md` —— 拒答阈值定多少、切分参数怎么调、改完到底变好没变好？靠感觉不行，得靠评测集和命中率。

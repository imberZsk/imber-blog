...
```bash
cd demos/19-api-design
python3 main.py
```

`main.py` 用中间件函数模拟请求过关，跑四类场景：

- `gen_request_id`：每个请求分配 requestId。
- `check_auth`：场景 1、2 演示匿名和错误 Key 被 401 挡掉。
- `check_rate_limit`：场景 3 里 alice 连发，第 4 次触发 429 限流。
- `handle_request`：把四道关按「requestId → 鉴权 → 限流 → 业务」串起来，每种失败返回对应状态码。

输出里能看到同样是失败，401、429、400 各不相同——这就是用对状态码的价值。

...
```bash
cd demos/40-logging-observability
python3 main.py
```

`main.py` 模拟三次 RAG 请求（命中、未命中、命中），把每次的 start/retrieve/generate/end 打成结构化 JSON 日志，最后聚合出命中率 67%、总 token、平均耗时。

核心对应关系：
- `StructuredLogger.log` —— 结构化日志的核心，JSON 一行一条
- `handle_request` 里的 `request_id` —— 串联一次请求的主线
- `fake_retrieve` / `fake_generate` —— 分阶段记录耗时、命中、token
- `print_metrics` —— 从日志聚合指标，体现可观测性的回报

跑完你能直观看到：散落的日志因为同一个 `request_id` 被串起来，又因为字段统一能被聚合。

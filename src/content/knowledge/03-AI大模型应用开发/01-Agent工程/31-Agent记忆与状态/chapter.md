...
```bash
cd demos/31-agent-memory
python3 main.py
```

`main.py` 演示两个会话，核心是 `ShortTermMemory` 类和 `load_long_term` / `save_long_term`：

- `session_one`：用户说出默认客户号，`ShortTermMemory` 记进会话，同时 `save_long_term` 持久化。随后多轮对话触发短期记忆截断——最早的「默认客户号是 C1001」被挤出短期记忆；
- `session_two`：全新会话，短期记忆是空的，但 `load_long_term` 把 C1001 读回来了，不用再问用户。

跑完你能清楚看到：短期记忆装不下被挤掉的东西，长期记忆持久化后跨会话还在。这就是两者的分工。
...
`32-Agent安全边界.md` —— Agent 能调工具、能访问数据、还能记住东西，一旦被恶意诱导风险就放大了。最后一篇讲怎么给 Agent 加安全护栏。

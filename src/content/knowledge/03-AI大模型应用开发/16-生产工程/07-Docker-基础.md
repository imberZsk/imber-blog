...
> 读完后，你应能完成以下任务：
> - 绘制“生产工程（07） - Docker 基础 / 镜像和容器：模板和实例”的关键对象与数据流，解释“包含系统、Python、依赖、你的代码。”，并用源码位置、日志或 Trace 标注证据。
> - 为“生产工程（07） - Docker 基础 / Dockerfile 逐行拆解”设计正常与异常输入，验证“slim 版比完整版小很多，又比 alpine 少踩 C 扩展编译的坑，是 Python 应用的稳妥选择。”，输出首个偏差位置与回归测试结果。
> - 实现“生产工程（07） - Docker 基础 / 为什么"先拷依赖再拷代码"——层缓存”的最小代码或配置，检验“第 3-5 行的顺序不是随便排的，这是 Docker 最重要的提速技巧。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **镜像和容器：模板和实例**：包含系统、Python、依赖、你的代码。
- **Dockerfile 逐行拆解**：slim 版比完整版小很多，又比 alpine 少踩 C 扩展编译的坑，是 Python 应用的稳妥选择。
- **为什么"先拷依赖再拷代码"——层缓存**：第 3-5 行的顺序不是随便排的，这是 Docker 最重要的提速技巧。
- **不装 Docker 也能先验证 app 本身**：main.py 是个最小 HTTP 服务，访问返回一句带 hostname 的 JSON。
- **工程上真正会踩的坑**：容器内必须监听 0.0.0.0（demo 里就是这么写的）。
- **一句话面试答法**：AI 应用依赖多、版本敏感，最容易出"我电脑能跑线上不能跑"。
...
USER 65532:65532
EXPOSE 8038
HEALTHCHECK --interval=30s --timeout=3s --retries=3 CMD python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8038/health', timeout=2)"
CMD ["python", "main.py"]
```

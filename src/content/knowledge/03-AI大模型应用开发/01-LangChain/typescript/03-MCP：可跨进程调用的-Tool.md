...
安装 `@langchain/mcp-adapters`，用 `MultiServerMCPClient` 配置本地 `stdio` 子进程或远程 Streamable HTTP 服务，再用 `getTools()` 得到 LangChain Tools。stdio 关注 `command`、`args`、`env`；HTTP 关注 `url`、认证、超时与重连。

```typescript runnable file=main.ts title="TypeScript MCP 传输选择" description="根据工具部署位置选择 stdio 或 HTTP。"
...
`@langchain/mcp-adapters` 的 `MultiServerMCPClient` 可以按传输配置连接多个 MCP Server，再用 `getTools()` 得到可交给 LangChain Agent 的工具集合。适配器只负责协议转换，不会替你放宽 MCP Server 的权限；每个工具仍要做白名单、参数校验和审计。
...
```typescript runnable file=main.ts title="TypeScript MCP 生命周期" description="验证 initialize、tools/list 和 tools/call 的顺序边界。"

...
TypeScript 可用 `JsonOutputParser` 处理增量 JSON，用 Zod 在最终提交前做完整校验。流式片段只用于预览，只有最终对象通过 schema 后才能写入业务系统。

```typescript runnable file=main.ts title="TypeScript 本篇最小实验" description="运行本篇 TypeScript 核心数据流。"
const chunks = ['{"title":', '"修复登录"}']
...
```typescript runnable file=main.ts title="TypeScript 流式预览与提交" description="累积 JSON 分块，证明完整校验前不会提交业务数据。"
/** 模型流式返回的 JSON 文本块。 */
const chunks = ['{"action":"create_reminder",', '"time":"2026-08-21T09:00:00Z"}']
/** 预览区当前累积的原始文本。 */
let preview = ''
/** 只有完整 JSON 通过业务校验后才变为 true。 */
let committed = false
...
/** 完整模型输出解析后的候选动作。 */
const candidate = JSON.parse(preview) as { action?: string; time?: string }
committed = candidate.action === 'create_reminder' && !Number.isNaN(Date.parse(candidate.time || ''))

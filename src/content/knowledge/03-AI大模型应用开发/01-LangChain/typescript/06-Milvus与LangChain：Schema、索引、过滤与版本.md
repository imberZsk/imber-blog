...
TypeScript 使用 `@langchain/milvus` 的 `Milvus` VectorStore。Milvus 层级是 database -> collection -> entity；collection 固定 schema，向量字段维度必须与 Embeddings 一致并建立索引，标量 metadata 用于租户和权限过滤。

```typescript runnable file=main.ts title="TypeScript Milvus schema 检查" description="验证 Embedding 维度与 collection schema。"
const schema = { vectorField: 'vector', dimension: 1536, indexType: 'HNSW' }
const embeddingDimension = 1536
if (schema.dimension !== embeddingDimension) throw new Error('Embedding 维度与 schema 不一致')
console.log(schema)
```
...
客户端 API 会随版本演进，
生产项目要锁定 `@zilliz/milvus2-sdk-node` 与 Milvus 兼容版本，
并用契约测试验证建表和查询。
...
- **Collection Schema**：客户端 API 会随版本演进，生产项目要锁定 `@zilliz/milvus2-sdk-node` 与 Milvus 兼容版本，并用契约测试验证建表和查询。
...
```typescript runnable file=main.ts title="TypeScript Milvus Schema 契约" description="验证 database、collection、entity、向量维度和索引配置。"
/** 教学 Collection 要求的向量维度。 */
const VECTOR_DIMENSION = 4
/** 等待写入 Collection 的单条 Entity。 */
const entity = { id: 'doc-1', tenantId: 'tenant-a', vector: [0.1, 0.2, 0.3, 0.4] }
/** 向量字段使用的近似检索索引。 */
const vectorIndex = { field: 'vector', indexType: 'HNSW', metricType: 'COSINE' }
if (entity.vector.length !== VECTOR_DIMENSION) throw new Error('vector dimension mismatch')
console.log({ database: 'knowledge', collection: 'book_chunks', entity, vectorIndex })
```

真实实验再把内存对象替换为 `@langchain/milvus` 或 `@zilliz/milvus2-sdk-node` 客户端，并用同一 Embeddings 模型保证向量维度一致。

# LangChain（04） - 用 Embeddings 把文档向量化并检索

## TypeScript 实现地图

TypeScript 用 `@langchain/core/documents` 的 `Document` 和 `@langchain/openai` 的 `OpenAIEmbeddings`。VectorStore 使用 `fromDocuments()` 入库、`asRetriever({ k })` 指定 Top K、`similaritySearchWithScore()` 查看评分，并通过 `retriever.invoke()` 查询。

```typescript runnable file=main.ts title="TypeScript Top K 检索" description="运行无网络的相似度排序实验。"
const documents = [{ text: 'Milvus 保存向量', score: 0.91 }, { text: 'Splitter 切分文档', score: 0.73 }]
console.log(documents.sort((left, right) => right.score - left.score).slice(0, 1))
```
...
本文把 `Document`、Embeddings 和 Retriever 串起来，重点是 LangChain 的 VectorStore API，不展开 Loader、Splitter 和 Milvus 的部署细节。课程顺序是：先理解向量化，再学习文档切分，最后接入 Milvus。

## 1.1 先记住四个 API

| API | 作用 | 关键边界 |
| --- | --- | --- |
| `fromDocuments` | 用 Embeddings 为一批 `Document` 计算向量并写入 VectorStore | 不是只保存原文，向量维度必须固定 |
| `asRetriever` | 把 VectorStore 包装成 Retriever，并指定 `k` 等召回参数 | `k` 是候选数量，不等于最终答案数量 |
| `similaritySearchWithScore` | 直接返回文档和相似度评分 | 分数含义取决于 VectorStore 和距离度量 |
| `retriever.invoke` | 用统一 Runnable 接口查询相关文档 | 查询文本会经过同一个 Embeddings 模型编码 |

`fromDocuments` 需要先准备 Embeddings 实例和 `Document[]`；如果文档没有稳定的 `pageContent`、`metadata.source` 和块 ID，后面即使召回成功也难以引用和排障。

本篇保留为 embedding 原理基础：重点讲文本转向量和余弦相似度。后续 Milvus 课程会把向量化放进入库、metadata 和生产索引中，再由最小 RAG 实战串成完整链路。
...
- **以为相似度高就一定答得对**。embedding 只负责「找出字面/语义相关的内容」，它不理解对错。相关不等于正确，因此生产链路还需要重排和检索评测。

...
Python v1 以 `create_agent` 和 Middleware 为高层入口，复杂状态进入 `langgraph`。迁移时逐项核对 PyPI 包、snake_case API、Pydantic schema 与 Python 版本。

```python runnable file=main.py title="Python 本篇最小实验" description="运行本篇 Python 核心数据流。"
migration_checks = ["package", "import path", "schema", "runtime"]
print(all(migration_checks))
```


> 读完后，你应能完成以下任务：
> - 绘制“LangChain（14） - LangChain v1 运行时、中间件与迁移边界 / 三种编排方式”的关键对象与数据流，解释“模型需要在有限工具间循环选择时使用 create_agent；”，并用源码位置、日志或 Trace 标注证据。

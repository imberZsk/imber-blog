...
Python 用 `langchain_core.documents.Document` 和 `langchain_openai.OpenAIEmbeddings`。VectorStore 使用 `from_documents()`、`as_retriever(search_kwargs={"k": ...})`、`similarity_search_with_score()` 与 `retriever.invoke()`。

```python runnable file=main.py title="Python Top K 检索" description="运行无网络的相似度排序实验。"
documents = [{"text": "Milvus 保存向量", "score": 0.91}, {"text": "Splitter 切分文档", "score": 0.73}]
print(sorted(documents, key=lambda item: item["score"], reverse=True)[:1])
```

...
Python 服务端可用 FastAPI `StreamingResponse` 输出 SSE，把 ChatModel 的 `stream()`/`astream()` 片段映射成事件；客户端断连时同样要取消上游任务。

```python runnable file=main.py title="Python 本篇最小实验" description="运行本篇 Python 核心数据流。"
import json
for chunk in ["你", "好"]:
    print(f"data: {json.dumps({'type': 'token', 'chunk': chunk}, ensure_ascii=False)}\n")
```

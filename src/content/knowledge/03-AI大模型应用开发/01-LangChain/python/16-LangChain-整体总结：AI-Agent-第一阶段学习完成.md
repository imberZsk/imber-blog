...
Python 路线应能串起 `langchain-core`、供应商包、`langchain-text-splitters`、`langchain-milvus`、`langchain-mcp-adapters`、LCEL、Memory 与 Agent，并能说明 Pydantic、snake_case API 和 Python 部署边界。

```python runnable file=main.py title="Python 本篇最小实验" description="运行本篇 Python 核心数据流。"
modules = ["messages", "tools", "mcp", "splitter", "vectorstore", "rag", "memory", "agent"]
print(f"已完成 {len(modules)} 个模块：{' -> '.join(modules)}")
```

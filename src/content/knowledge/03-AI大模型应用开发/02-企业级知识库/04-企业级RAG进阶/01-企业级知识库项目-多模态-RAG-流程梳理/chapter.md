...
> 主分类：企业级知识库；关联标签：工程基础、RAG 优化、多模态、安全合规
...
> DIAGRAM_DESCRIPTION：架构图必须展示对象存储安全入口、版面解析/OCR/ASR、统一元素模型、结构化 Chunk、ES、文本/图像向量、原始视觉证据、多路检索、Rerank、VLM/LLM 和引用复核；ACL 必须进入检索而非只在答案末尾检查。
...
> SCREENSHOT_DESCRIPTION：配图应展示一次真实 Trace 的四个区域：用户问题与权限摘要、多路召回候选及来源模态、Rerank 前后排名、最终答案引用回跳后的 PDF 页码与图片裁剪；截图必须脱敏，隐藏密钥、租户名称和原始敏感正文。

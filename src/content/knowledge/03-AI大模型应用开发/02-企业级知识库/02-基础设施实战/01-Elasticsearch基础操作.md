# 基础设施实战（1）- Elasticsearch 知识库索引：BM25、权限与零停机发布

> 主分类：企业级知识库；关联标签：工程基础、BM25、混合检索
...
> DIAGRAM_DESCRIPTION：架构图必须包含规范化 Chunk、批量写入、版本化索引、BM25、权限字段、查询权限过滤、读别名和候选输出；权限过滤必须与检索一起执行。

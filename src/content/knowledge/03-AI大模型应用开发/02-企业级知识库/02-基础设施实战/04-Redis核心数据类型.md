# 基础设施实战（4）- Redis 在 AI 系统中的边界：短期记忆、缓存与限流

> 主分类：企业级知识库；关联标签：LangChain 实战、Memory、Redis
...
> DIAGRAM_DESCRIPTION：架构图必须展示 Redis 承担会话窗口、检索缓存、限流和幂等；长期记忆进入 PostgreSQL 或 Mem0，知识检索仍由 ES/VectorDB 提供，避免把 Redis 画成唯一事实源。

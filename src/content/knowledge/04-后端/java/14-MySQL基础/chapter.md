...
| --- | --- |
| `users.filter(u => u.city === '北京')` | `SELECT * FROM company WHERE city = '北京'` |
| `users.sort((a,b) => b.time - a.time)` | `ORDER BY create_time DESC` |
| `users.slice(0, 10)` | `LIMIT 10` |
| `users.find(u => u.id === '123')` | `SELECT * FROM company WHERE id = '123'` |
...
## 二、先认识一张真实的表：cyt 的 `company` 网点表

在 cyt 项目里，所有"公司/网点"信息都存在 `company` 这张表里。我们从 `cyt-basic-service/src/main/resources/mapper/CompanyMapper.xml` 能反推出它的字段。挑几个核心字段，建表语句大致长这样（贴近 cyt 真实风格）：

```sql
-- company 网点表（货运人公司/网点基础信息）
...
> 【前端类比】`CREATE TABLE` 就像你用 TypeScript 定义一个 `interface Company { id: string; companyName: string; ... }`，再规定"这个数组里每个元素都必须长这样"。区别是 SQL 还要声明每个字段的**存储类型和长度**（`VARCHAR(128)`），以及哪些字段建了**索引**（查得快）。

注意 Java 实体 `Company` 的字段名是**驼峰** `companyName`，而表字段是**下划线** `company_name`——这个映射关系正是第 04 课讲的 Mapper 干的活（`<result property="companyName" column="company_name"/>`）。
...
SELECT id, company_name, city, status FROM company;
```

> 【前端类比】`SELECT *` ≈ 把整个对象都拿过来；`SELECT id, company_name` ≈ 解构只取你要的字段 `const { id, companyName } = company`。少拿字段 = 少传输 = 更快，后端尤其讲究这个。

### 3.2 INSERT —— 增（Create）

```sql
-- 插入一条网点记录，列名和值一一对应
INSERT INTO company (id, company_name, city, status, create_time)
VALUES ('1001', '北京朝阳网点', '北京', 1, NOW());
```

> 【前端类比】≈ `arr.push({ id: '1001', companyName: '北京朝阳网点', ... })`。`NOW()` 是 MySQL 内置函数，返回当前时间，相当于 JS 的 `new Date()`。
...
> ⚠️ 同样，`DELETE FROM company` 不带 WHERE = 清空整张表。实际业务里 cyt 很少物理删除，更多是上面那种 `UPDATE status = 0` 的**逻辑删除**（软删除），数据留痕方便追溯。
...
```sql
-- 模糊查：公司名以"北京"开头
SELECT * FROM company WHERE company_name LIKE '北京%';

-- IN 查询：查这几个城市的网点
SELECT * FROM company WHERE city IN ('北京', '上海', '广州');
```
...
```sql
-- 按创建时间倒序（最新的在前）
SELECT id, company_name, create_time
FROM company
...
```sql
-- 取前 10 条
SELECT * FROM company ORDER BY create_time DESC LIMIT 10;

-- 分页：跳过前 20 条，再取 10 条（即第 3 页，每页 10 条）
SELECT * FROM company ORDER BY create_time DESC LIMIT 20, 10;
--                                                    ↑offset ↑每页条数
```

> 【前端类比】`LIMIT 20, 10` ≈ `arr.slice(20, 30)`。在 cyt 里你几乎不用手写 `LIMIT`——用的是 PageHelper / MyBatis-Plus 这类分页插件，你只管传 `pageNum`、`pageSize`，它自动帮你拼 `LIMIT`。
...
## 五、动态拼 SQL：看 cyt 真实代码

前面的 SQL 都是写死的。但真实业务里，查询条件是用户传的——可能传了城市，也可能没传。这时就要"动态拼 SQL"。

看 cyt `CompanyMapper.xml` 里 `getCompanyPage` 的真实片段（节选）：

```xml
<!-- 来自 cyt-basic-service/.../mapper/CompanyMapper.xml -->
<select id="getCompanyPage" resultMap="companyMap">
  SELECT id, company_name, company_code, city, status, create_time
  FROM company
  <where>
    <!-- 只有当传了 companyName 才拼这个条件，避免污染 SQL -->
    <if test="company.companyName != null and company.companyName.trim() != ''">
      AND company_name = #{company.companyName}
    </if>
    <!-- 只有传了 city 才按城市过滤 -->
    <if test="company.city != null and company.city.trim() != ''">
      AND city = #{company.city}
    </if>
    <if test="company.status != null and company.status.trim() != ''">
      AND status = #{company.status}
    </if>
  </where>
</select>
```
...
`#{company.city}` 这种 `#{}` 写法是**参数占位符**，MyBatis 会用预编译（`?`）方式塞值进去——**这是防 SQL 注入的关键**，千万别用字符串拼接把用户输入直接接到 SQL 里。
...
业务场景：`company` 表里确实存了 `group_id`（集团ID，这是 cyt `company` 表的真实字段），但表里没存集团的名字。假设集团名字存在另一张表 `company_group`（这张表名是为讲清 JOIN 概念虚构的示意表，cyt 实际未必这么拆），你想一次查出"网点名 + 它所属集团的名字"，就要 **JOIN**（连表）。

```sql
-- 查网点及其所属集团名称（company_group 为示意表）
SELECT
    c.company_name,        -- 网点名（来自 company 表，别名 c）
    c.city,
    g.group_name           -- 集团名（来自示意表 company_group，别名 g）
FROM company c
INNER JOIN company_group g ON c.group_id = g.id   -- 关联条件：用 group_id 对上
WHERE c.status = 1;
```

> 【前端类比】JOIN ≈ 你拿到两个数组，手动做关联：
> ```js
> // 前端手动 join：给每个 company 找到它的 group
> companies.map(c => {
>   const g = groups.find(g => g.id === c.groupId);
>   return { companyName: c.companyName, groupName: g?.groupName };
> });
> ```
...
```sql
-- 即使某网点没绑定集团（group_id 为空），这条网点也要查出来（company_group 为示意表）
SELECT c.company_name, g.group_name
FROM company c
LEFT JOIN company_group g ON c.group_id = g.id;
-- 没匹配上的 g.group_name 会是 NULL
```

> ⚠️ 注意：JOIN 虽强，但连的表越多、数据量越大越慢。cyt 实际代码里很多场景**故意不 JOIN**，而是先查一张表拿到 ID 列表，再用 `IN` 查另一张表，在 Java 内存里组装——这样每条 SQL 都简单、走得动索引，整体反而更快、更好维护。该不该 JOIN 是后端的一项重要权衡。
...
假设 `company` 表有 100 万行。执行：

```sql
SELECT * FROM company WHERE city = '北京';
```
...
- **WHERE / JOIN ON / ORDER BY 用到的字段**，是建索引的首选。比如 cyt 的 `company` 表，`group_id`、`city` 经常被当查询条件，就该建索引。
- **主键（PRIMARY KEY）自带索引**，所以 `WHERE id = '1001'` 永远很快——这也是为什么 cyt 大量接口都是按 id 查（见第 04 课 `getById`）。
- **区分度低的字段别单独建索引**：比如 `status` 只有 0/1 两个值，建索引意义不大（类比：按"性别"建目录，翻一半也没省多少）。
- 用 `EXPLAIN` 看一条 SQL 有没有走索引：

```sql
EXPLAIN SELECT * FROM company WHERE city = '北京';
...
```sql
UPDATE company SET settle_remainder = settle_remainder - 100 WHERE id = 'A';  -- A 减 100
UPDATE company SET settle_remainder = settle_remainder + 100 WHERE id = 'B';  -- B 加 100
```
...
UPDATE company SET settle_remainder = settle_remainder - 100 WHERE id = 'A';
UPDATE company SET settle_remainder = settle_remainder + 100 WHERE id = 'B';
...
public void settle(String fromId, String toId, BigDecimal amount) {
    // 第一步：转出方扣钱
    companyMapper.decreaseRemainder(fromId, amount);
    // 第二步：转入方加钱——若这里抛异常，上一步的扣钱也会被自动回滚
    companyMapper.increaseRemainder(toId, amount);
}
```
...
### 8.4 并发下的"加锁"：cyt 真实代码

并发场景下，为了保证隔离性，常用"行锁"。看 cyt `CompanyMapper.xml` 里这条真实 SQL：

```xml
<!-- 来自 cyt-basic-service/.../mapper/CompanyMapper.xml -->
<select id="selectCompanyForUpdate" resultType="...Company">
    SELECT * from company WHERE id = #{id} for update
</select>
```
...
- **CRUD 四板斧**：`SELECT`（查）/`INSERT`（增）/`UPDATE`（改）/`DELETE`（删）。改和删**务必先写 WHERE**，否则全表遭殃。cyt 多用 `UPDATE status=0` 做逻辑删除。
- **查询三修饰**：`WHERE`（过滤，≈filter）→ `ORDER BY`（排序，≈sort）→ `LIMIT`（分页，≈slice），执行顺序也是这个。
- **动态 SQL**：cyt 的 `getCompanyPage` 用 `<if>`+`<where>` 按需拼条件，`#{}` 占位符防注入。
- **JOIN** 多表关联：`INNER JOIN`（两边都匹配）/`LEFT JOIN`（左表全留，最常用）。≈ 前端手动 `find` 关联，但更快。该不该 JOIN 是后端权衡，cyt 常拆成多条简单查询。
- **索引** ≈ 给数组建 Map/字典目录，让查询从 O(n) 全表扫描降到 O(log n)。代价是写入变慢、占空间。WHERE/JOIN/ORDER BY 的字段适合建索引，主键自带索引。
- **事务 ACID**：原子性/一致性/隔离性/持久性。Java 用 `@Transactional` 注解管理，记得 `rollbackFor = Exception.class`。并发场景用 `SELECT ... FOR UPDATE` 加行锁（cyt 真实在用）。

下一课预告：**第 15 课 MyBatis 进阶**——我们会深入这一课反复出现的 Mapper XML，搞懂 `resultMap` 字段映射、`#{}` 与 `${}` 的本质区别、`<foreach>` 批量操作，以及 cyt 是怎么把 SQL 和 Java 方法优雅地绑在一起的。

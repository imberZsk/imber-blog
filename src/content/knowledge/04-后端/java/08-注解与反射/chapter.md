...
> 注解（@xxx）是 Spring 的基础。你在 cyt 代码里见过的 @RestController、@Autowired 全是注解。
...
@RequestMapping("/company")  // 标记：处理 /company 请求
...
```javascript
axios.get('/company/getById?id=123')  // 参数在 URL 上
```
...
```javascript
axios.post('/company/getByGidWithoutDel', {  // 参数在请求体
    groupId: 1,
    ids: [1, 2, 3]
})
```
...
cyt 大量用 Lombok 自动生成代码。最常见的 `@Data`：
...
类比：就像 TS 不用手写一堆样板代码。`@Data` 帮你把 getter/setter 全生成了，所以 cyt 的实体类看起来很干净，只有字段。
...
- **Lombok**（`@Data`/`@Slf4j`）自动生成样板代码，所以 cyt 实体类很干净

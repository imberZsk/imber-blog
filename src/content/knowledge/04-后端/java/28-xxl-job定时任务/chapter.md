...
假设 cyt 的财务服务 `cyt-finance` 要做一个"每天凌晨对账校验"的任务（真实存在，见后文 `RemainderCheckJob`）。如果你直接用 Spring 的 `@Scheduled(cron = "0 0 2 * * ?")` 写在代码里，会遇到下面这些问题。

### 坑 1：多实例重复执行

后端服务为了高可用，从来不是只跑一个进程。`cyt-finance` 线上至少 3 个实例（Pod）。`@Scheduled` 是"每个进程各跑各的"：
...
cyt 有几十万辆车要检查保险到期（真实任务 `InsuranceExpireNotifyJob`）。单机一个进程从头跑到尾，几个小时跑不完。你希望 3 个 Pod 一人分一部分数据并行跑，单机 cron 做不到。
...
```
 ...
 └────────────────────────┬─────────────────────────────────┘
            ...
            ▼             ▼             ▼
     ┌───────────┐ ┌───────────┐ ┌───────────┐
     │ 执行器 Pod1 │ │ 执行器 Pod2 │ │ 执行器 Pod3 │   ← 你的业务服务（cyt-finance）
...
| **执行器 (executor)** | 你的业务服务（`cyt-finance`、`cyt-basic`），内嵌一个 xxl-job 客户端 | 真正干活的 `() => { ... }` 回调函数所在的进程 |
...
业务服务要变成"执行器"，得在启动时往调度中心注册自己。cyt-finance 里这段配置（真实代码 `cyt-finance/.../config/XxlJobConfig.java`）：
...
public class XxlJobConfig {
    ...
    // 执行器分组名，比如 "cyt-finance-executor"
...
1. **`adminAddresses`**：执行器知道调度中心在哪，启动时主动连过去说"我是 cyt-finance 的一个 Pod，有任务可以派给我"。这叫**自动注册**——你扩容到 5 个 Pod，调度中心自动就看到 5 个，不用手配 IP。
...
cyt-basic 里这个"从油品同步货物名称类型"的同步任务（真实代码 `cyt-basic/.../jobhandler/SyncCargoTypeJobHandler.java`），是 xxl-job 最朴素的样子：
...
## 七、真实场景一：对账校验任务（解决"坑 1 重复执行"）

cyt-finance 的 `RemainderCheckJob`（真实代码 `cyt-finance/.../schedule/RemainderCheckJob.java`）是个典型的"对账"任务：每天定时把所有网点的账算一遍，和数据库里存的余额对比，对不上就报警。
...
@Slf4j
@Component
public class RemainderCheckJob extends IJobHandler {  // 另一种写法：继承 IJobHandler，重写 execute
    ...
    public ReturnT execute(String param) {
        ...
        try {
            ...
            Set<CompanyUserProVo> checkedGroup = new HashSet<>();
            queryCompanyAndCheck(0, jobParam, checkedGroup);   // 递归分页查网点 + 逐个校验
...
- **两种写法都行**：`SyncCargoTypeJobHandler` 是"普通类 + `@XxlJob` 注解方法"；`RemainderCheckJob` 是"继承 `IJobHandler` + 重写 `execute`"。新代码推荐前者（更轻量），老任务常见后者。

**为什么不会重复跑？** 调度中心的路由策略默认是"第一个/轮询/一致性哈希"等——一次触发只选**一个**活着的 Pod 发信号。3 个 Pod 注册在同一个 `appname` 下，但每次只有一个被选中执行。坑 1 解决。

---

## 八、真实场景二：大数据量任务的分片（解决"坑 4 拆不开"）

cyt-vehiclebiz 的保险到期提醒 `InsuranceExpireNotifyJob`（真实代码 `cyt-vehiclebiz/.../job/InsuranceExpireNotifyJob.java`）。全公司几十万辆车，单机跑太慢，于是用 xxl-job 的**分片广播**：让 3 个 Pod **同时**跑，每个 Pod 只处理"自己那一份"数据。

```java
@Component
@RequiredArgsConstructor
public class InsuranceExpireNotifyJob extends BaseLogJob {   // 继承封装好的分片基类
    ...
    public void exec(int total, int index) {
        ...
        // 注意：cyt 这里是按"集团"分片，不是按单辆车分片——一个 Pod 包揽它负责的那些集团的全部车
        expireNotifyService.expireNotify(total, index, null);
    }
}
```

分片参数从哪来？看封装好的基类 `BaseLogJob`（真实代码 `cyt-vehiclebiz/.../job/BaseLogJob.java`）：
...
> 前端类比：想象你有一堆集团要处理，开了 3 个 Web Worker，主线程告诉每个 Worker "你是 3 个里的第 N 个，只处理你那一份"。`total` 是 Worker 总数，`index` 是当前 Worker 编号。xxl-job 帮你把 `total/index` 算好下发，你只管按它取模分活。cyt 里取模的对象是"集团 id"——每个 Pod 认领一批集团，把这些集团下的车全部处理掉。

注意"分片广播"和第七节"对账"的关键区别：

| 任务类型 | 路由策略 | 几个 Pod 执行 | 适用场景 |
|----------|----------|---------------|----------|
| 对账校验 `RemainderCheckJob` | 轮询/第一个等 | **只 1 个** | 任务本身不大，绝不能重复 |
| 保险提醒 `InsuranceExpireNotifyJob` | **分片广播** | **全部一起**，各管一份 | 数据量大，要并行加速 |
...
②  凌晨 2:00 到点，调度中心按 cron 触发
            │
            ▼
③  按路由策略，从 cyt-finance 这组注册的 3 个 Pod 里挑 1 个
...
- **执行器接入**靠 `XxlJobConfig` 里的 `XxlJobSpringExecutor`：用 `adminAddresses` 注册到调度中心，用 `appname` 分组，启动时自动上线（cyt-finance 真实配置）。
- **写任务**：`@Component` + `@XxlJob("任务名")` 标在方法上，方法返回 `ReturnT.SUCCESS/FAIL` 向调度中心交差（cyt-basic 的 `SyncCargoTypeJobHandler`）。或继承 `IJobHandler` 重写 `execute`（cyt-finance 的 `RemainderCheckJob`）。
- **cron 表达式**和前端 node-cron 几乎一样，只是 xxl-job 是 6 位、最前面多一位"秒"。
- **路由策略决定几个 Pod 执行**：对账类选"轮询/第一个"只跑 1 个防重复；大数据量选"分片广播"，靠 `ShardingUtil` 拿到的 `total/index` 取模，多 Pod 并行各管一份（cyt-vehiclebiz 的 `InsuranceExpireNotifyJob` + `BaseLogJob`）。
...
**下一课预告**：第 29 课我们讲 **消息队列（RocketMQ/Kafka）入门**——定时任务是"按时间触发"，消息队列是"按事件触发、异步解耦"。我们会对比前端的事件总线（EventBus / 发布订阅）来理解"生产者-消费者"模型，并结合 cyt 真实的下单后异步发消息场景讲清楚为什么要引入 MQ。

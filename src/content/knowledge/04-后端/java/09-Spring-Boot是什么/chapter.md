...
CompanyMapper mapper = new CompanyMapper();          // 自己造 Mapper
DriverCompanyService driverSvc = new DriverCompanyService(); // 自己造依赖
CompanyService service = new CompanyService(mapper, driverSvc); // 手动塞进去
CompanyController controller = new CompanyController(service);  // 一层层往上拼
// 如果 DriverCompanyService 自己又依赖 3 个对象呢？嵌套地狱。
```
...
靠注解（第 08 课讲过注解是「贴在代码上的标签」）。看 cyt 真实代码 `CompanyService`：

```java
// 文件：cyt-basic/cyt-basic-service/.../service/CompanyService.java
@Slf4j                         // Lombok：自动生成日志对象 log（见第 08 课）
@Service("companyService")     // 关键：告诉 Spring「我是一个 Bean，名字叫 companyService」
...
| `@Service` | 业务逻辑层（如 CompanyService） | service 层模块 |
...
看 cyt 真实代码，`CompanyService` 里那几个 `@Autowired`：

```java
@Service("companyService")
public class CompanyService extends ServiceImpl<CompanyMapper, Company> {

    // @Autowired：向容器说「我需要 DriverCompanyService 这个 Bean，请注入给我」
    ...
    // ...业务方法里直接 driverCompanyService.xxx() 用，从没出现过 new
}
```

注意：整个 `CompanyService` 里**没有一行 `new DriverCompanyService()`**。它只是「声明需要」，容器在创建 `CompanyService` 这个 Bean 时，发现它要 `DriverCompanyService`，就从池子里找出来塞进去。这就是 DI。
...
应用启动
   ...
   │     → new CompanyService()、new DriverCompanyService() ... 放进容器
   │
   ├─ 创建 CompanyService 时发现它有 @Autowired DriverCompanyService
   │     → 去容器里找 DriverCompanyService 这个 Bean
   │     → 找到，塞进 CompanyService 的字段  ✅ 注入完成
   │
...
```xml
<!-- 文件：cyt-basic/cyt-basic-biz/pom.xml -->
...
- 启动一个内嵌 Web 服务器（默认 Tomcat，cyt 这里换成了 Undertow）
- 配好 JSON 的序列化/反序列化
- 注册好处理请求的各种组件
...
回到 cyt 真实的启动类，它就是「打开自动配置开关」并启动容器的地方：

```java
// 文件：cyt-basic/cyt-basic-biz/.../CytBasicAdminApplication.java
@EnableFeignClients(basePackages = {"com.huoyunren.cyt"})  // 开启 Feign 远程调用扫描
@ComponentScan({"com.huoyunren.cyt"})                       // 指定扫描哪些包来找 Bean
...
几个关键点：
- `@ComponentScan({"com.huoyunren.cyt"})`：告诉容器「去 `com.huoyunren.cyt` 这个包下面扫，凡是带 `@Service`/`@Component` 等标签的都登记成 Bean」。这就是为什么 `CompanyService` 不用注册就能被注入——它在被扫描的包里。
...
│ 2. @ComponentScan 扫描 com.huoyunren.cyt 包     │
...
| 概念 | 一句话定义 | 前端类比 | cyt 真实例子 |
|------|------------|----------|--------------|
| IoC 控制反转 | 把「创建/装配对象」的控制权交给框架 | Vue 帮你管组件生命周期 | 整个 cyt-basic 应用 |
| 容器 | 装并管理所有 Bean 的池子 | Pinia store 注册表 / app 实例 | `SpringApplication.run` 建立的容器 |
| Bean | 被容器创建和管理的对象 | 被托管的 store 实例（默认单例） | `@Service("companyService")` 的 CompanyService |
| DI 依赖注入 | 声明需要即自动塞入，不用自己 new | Vue `provide/inject` | `@Autowired DriverCompanyService` |
| starter 起步依赖 | 一个全家桶带来配套依赖 | 装 `vite` 带来一堆配套 | `spring-boot-starter-web` |
| 自动配置 | 按引入的 starter 自动配置好 | Vite 零配置开箱即用 | 引 web starter 即自动起服务器 |
| 启动类 | 打开开关、启动容器的入口 | `createApp(App).mount()` | `CytBasicAdminApplication` |
...
- **DI（依赖注入）** 是 IoC 的落地手段，也就是第 05 课 `@Autowired` 的真相：只「声明需要」，容器自动把对应 Bean 塞进来，全程不用 `new`——cyt 的 `CompanyService` 里三个 `@Autowired` 字段就是活例子。
...
> **下一课预告**：第 10 课《Spring Boot 项目结构与分层》。我们会把第 04 课那条 Controller→Service→Mapper 的请求链路，对应到 cyt 真实的多模块工程结构（你已经见过 cyt-basic 拆成 biz/service/client/common 四个模块了），讲清每一层的职责、为什么要这样分层，以及前端工程师该如何快速读懂一个陌生的 Java 后端项目目录。

...
所以你已经有直觉了：**MyBatis 偏"手写 SQL 派"，MyBatis-Plus 偏"自动生成派"**。cyt 项目里两者混用——简单 CRUD 用 Plus 一行搞定，复杂查询落到 XML 手写 SQL。
...
来看 cyt-basic 的真实代码 `CompanyMapper.java`：

```java
// 文件：cyt-basic-service/.../mapper/CompanyMapper.java
package com.huoyunren.cyt.basic.service.mapper;
...
/**
 * 物流公司表 Mapper
 * 关键：继承 BaseMapper<Company> 就白送了一堆 CRUD 方法（下面第三节细讲）
 */
public interface CompanyMapper extends BaseMapper<Company> {
   ...
   * @param company 查询条件载体，把前端传来的筛选字段塞进这个对象
   * @return 分页结果，IPage 里既有当页数据也有总条数
   */
  IPage<Company> getCompanyPage(Page page, @Param("company") Company company);
}
```

> 注意：这里**没有 `CompanyMapperImpl`**。你不会找到任何一个类去 `implements CompanyMapper`。

**那实现在哪？** MyBatis 在启动时用第 08 课讲过的"反射 + 动态代理"，自动给这个接口生成了一个隐形的实现类：

```
你写的接口         MyBatis 偷偷生成的代理对象
CompanyMapper  →  [运行时动态代理] → 找到同名 XML 里的 SQL → 执行 → 把结果映射回 Company
```
...
interface CompanyApi {
  getCompanyPage(page: Page, company: Company): Promise<IPage<Company>>;
}
// ❌ 这个接口不能直接调用，必须有人 implements

// Java MyBatis：接口声明完，实现由框架在运行时"自动注入"
// ✅ @Autowired CompanyMapper 直接就能 .getCompanyPage(...) 调用
```

更贴切的类比是 **GraphQL Code Generator**：你写好 schema/query，工具生成可直接调用的强类型函数。MyBatis 干的是同一件事，只是把"生成"放到了运行时。

`@Param("company")` 这个注解也要记一下：它给参数起个"在 SQL 里能用的名字"。后面 XML 里写 `#{company.companyName}` 时，`company` 这个前缀就来自这里。

---

### 2.2 XML 映射 —— SQL 写在这里

接口只有签名，那 SQL 本体呢？在 **同名 XML 文件**里。cyt 的约定是放在 `resources/mapper/CompanyMapper.xml`，靠 `namespace` 和接口绑死：
...
<mapper namespace="com.huoyunren.cyt.basic.service.mapper.CompanyMapper">

  <!-- resultMap：定义"数据库列 → Java 字段"的映射关系 -->
  <resultMap id="companyMap" type="com.huoyunren.cyt.basic.service.entity.Company">
    <id     property="id"          column="id"/>            <!-- 主键 -->
    <result property="groupId"     column="group_id"/>      <!-- 驼峰 groupId ← 下划线 group_id -->
    <result property="companyName" column="company_name"/>
    <result property="companyCode" column="company_code"/>
    <!-- ……其余字段省略，cyt 真实 XML 里有 60+ 个字段 -->
  </resultMap>

  <!-- select 的 id 必须 = 接口里的方法名 getCompanyPage -->
  <select id="getCompanyPage" resultMap="companyMap">
    SELECT id, group_id, company_name, company_code, short_name, type, state
    FROM company
    <!-- 动态 SQL 在这里，下一节讲 -->
  </select>
</mapper>
...
方法名 getCompanyPage ──绑定──→     <select id="getCompanyPage">
返回类型 Company       ──绑定──→     <resultMap type="...Company">
```
...
| 层 | 命名习惯 | 例子 |
| --- | --- | --- |
| MySQL 列名 | 下划线 snake_case | `company_name` |
| Java 字段 | 驼峰 camelCase | `companyName` |

`resultMap` 就是这张"翻译表"。

> **前端类比**：跟你用 axios 拦截器或 `humps` 库把后端返回的 `snake_case` 自动转成前端的 `camelCase` 是一模一样的诉求。只不过 MyBatis 把这层翻译写在了 XML 里。
>
> 好消息：MyBatis-Plus 默认开启了"驼峰自动映射"，所以**简单字段根本不用手写 resultMap**——`company_name` 会自动对上 `companyName`。cyt 这份 XML 写得这么全，是早期代码生成器一把梭生成的，实战中新表大多省掉。
...
来看 `getCompanyPage` 里真实的动态 SQL（节选）：

```xml
<select id="getCompanyPage" resultMap="companyMap">
  SELECT id, group_id, company_name, company_code, state
  FROM company
  <!-- <where> 标签很聪明：有条件才加 WHERE，且自动去掉开头多余的 AND -->
  <where>
    <!-- <if> = JS 里的 if。test 里写判断表达式 -->
    <!-- 这句的业务含义：只有当前端传了 companyName 且非空白，才拼这个过滤条件 -->
    <if test="company.companyName != null and company.companyName.trim() != ''">
      AND company_name = #{company.companyName}
    </if>
    <if test="company.companyCode != null and company.companyCode.trim() != ''">
      AND company_code = #{company.companyCode}
    </if>
    <if test="company.shortName != null and company.shortName.trim() != ''">
      AND short_name = #{company.shortName}
    </if>
  </where>
</select>
```

> 小坑提醒：`test` 表达式里的 `.trim()` 只能用在**字符串字段**上。cyt 这份 XML 是代码生成器一把梭生成的，连 `state`（`Integer` 类型）都写成了 `state.trim()`——这其实是个隐患，一旦 `state` 真有值，OGNL 解析 `.trim()` 会抛异常。判空数字字段直接写 `company.state != null` 就够了。

**这段 SQL 会根据传入条件"变形"**：

```
前端只传了 companyName           →  SELECT ... FROM company WHERE company_name = ?
前端传了 companyName + shortName →  SELECT ... FROM company WHERE company_name = ? AND short_name = ?
前端啥都没传                      →  SELECT ... FROM company   （<where> 发现没条件，连 WHERE 都不加）
```
...
if (companyName) params.companyName = companyName;  // 等价于 <if> + AND company_name=?
if (state) params.state = state;
const url = `/api/company?${qs.stringify(params)}`;
```

`<if test="...">` 就是 XML 版的 `if (companyName)`，`<where>` 帮你处理了"第一个条件前面不该有 AND"这种烦人的边界。

#### `#{}` vs `${}` —— 安全红线，必须分清

| 写法 | 行为 | 类比 | 安全性 |
| --- | --- | --- | --- |
| `#{company.id}` | 预编译占位符 `?`，参数单独传 | 参数化查询 / `db.query('...?', [id])` | ✅ 防 SQL 注入 |
| `${company.id}` | **字符串直接拼进 SQL** | 模板字符串 `` `...${id}` `` | ⚠️ 有注入风险 |

**默认永远用 `#{}`**。只有在拼"列名、表名、排序方向"这种没法参数化的位置才不得已用 `${}`，且必须确保来源可信（不能是用户直接输入）。这跟前端"绝不把用户输入直接拼进 innerHTML / SQL"是同一条铁律。

#### `foreach` —— 处理 `IN (...)` 列表

cyt 真实代码 `selectChildCompanies`（下面节选 `property in` 这一段，原方法后面还用第二个 `foreach` 拼了 `FIND_IN_SET`）用 `foreach` 把一个 List 展开成 `IN (...)`：

```xml
<select id="selectChildCompanies" resultType="com.huoyunren.cyt.basic.service.entity.Company">
  select * from company where property in
  <!-- foreach 遍历 properties 列表，用逗号分隔，前后加括号 -->
  <!-- 渲染结果形如：property in ( 1 , 2 )  -->
  <foreach item="item" collection="properties" separator="," open="(" close=")">
    #{item}
  </foreach>
  <!-- 真实代码后面还有：and <foreach ... separator="or"> FIND_IN_SET(#{item}, parent_ids) </foreach> -->
</select>
```
...
还记得 `CompanyMapper extends BaseMapper<Company>` 吗？这个 `extends` 就是魔法来源。继承 `BaseMapper<Company>` 后，**你一行 SQL 都没写，就白得了一整套 CRUD**：

```java
// 这些方法你没定义，但都能直接调用，因为 BaseMapper 已经实现了
companyMapper.selectById(123);                  // 按主键查一条 → Company
companyMapper.selectBatchIds(Arrays.asList(1,2,3)); // 按主键批量查 → List<Company>
companyMapper.selectList(wrapper);              // 按条件查多条 → List<Company>
companyMapper.selectOne(wrapper);               // 按条件查一条 → Company
companyMapper.insert(company);                  // 插入
companyMapper.updateById(company);              // 按主键更新
companyMapper.deleteById(123);                  // 按主键删除
```
...
await prisma.company.findUnique({ where: { id: 123 } });   // ≈ selectById(123)
await prisma.company.findMany({ where: { state: 2 } });    // ≈ selectList(wrapper)
```

MP 是怎么知道查哪张表、哪些列的？靠 entity 上的注解（第 08 课讲过注解）：

```java
// 文件：cyt-basic-service/.../entity/Company.java
@Data                              // Lombok：自动生成 getter/setter（第 08 课）
@EqualsAndHashCode(callSuper = true)
@TableName("company")              // 告诉 MP：这个类对应数据库 company 表
public class Company extends Model<Company> {
    ...
    private String  companyName;   // 物流公司名称（自动映射到列 company_name）
...
| 注解 | 作用 | 对应 Prisma |
| --- | --- | --- |
| `@TableName("company")` | 类 ↔ 表名 | `@@map("company")` |
| `@TableId` | 字段 ↔ 主键 | `@id` |
| （字段名驼峰自动转下划线） | `companyName` ↔ `company_name` | `@map("company_name")` |
...
Wrapper 让你用**链式 Java 方法**拼 WHERE 条件，完全不碰 SQL 字符串。看 cyt-basic 的真实代码 `MaterialRepositoryImpl.java`：

```java
// 文件：cyt-basic-service/.../repository/impl/MaterialRepositoryImpl.java
...
| 复杂 SQL（多表 JOIN、`FIND_IN_SET`、`JSON_SET` 等） | 手写 XML | `CompanyMapper.selectChildCompanies` |

判断口诀：**能用 Wrapper 别写 XML，但 SQL 一复杂（JOIN、函数、特殊语法）就老老实实回 XML。** cyt 的 `incrementUpdateSettleRemainderById` 用到了 `JSON_SET`、`COALESCE` 这种 Wrapper 表达不了的 MySQL 函数，就必须落在 XML 里。
...
```
前端发请求 GET /company/childList?ids=12,34
        │
        ▼
┌──────────────┐
│ Controller   │  接收参数 ids
...
┌──────────────────────────────────────────────┐
│ Mapper（接口，无实现类）                        │
│   companyMapper.selectChildCompanies(ids, ...) │
└──────┬─────────────────────────────────────────┘
       ▼  MyBatis 动态代理介入
┌──────────────────────────────────────────────┐
...
└──────┬─────────────────────────────────────────┘
       ▼
┌──────────────┐
│ MySQL        │  执行 SELECT * FROM company WHERE ...
└──────┬───────┘
       ▼  resultMap / 驼峰映射 把列名转回 Java 字段
┌──────────────┐
│ List<Company>│  Java 对象列表，原路返回给前端
└──────────────┘
```
...
- 引用的 cyt 真实代码：`CompanyMapper`（继承 BaseMapper + 自定义分页方法）、`CompanyMapper.xml`（`<where>/<if>/<foreach>` 动态 SQL）、`Company` entity（`@TableName/@TableId`）、`MaterialRepositoryImpl`（`Wrappers.lambdaQuery().eq().in()` 真实用法）。

**下一课预告**：第 16 课《事务与 @Transactional》——查询会了，但当一次业务要改多张表（比如同时扣网点余额 + 写流水），如何保证"要么全成功、要么全回滚"？我们会讲 Spring 声明式事务，对比前端很少直接打交道的 ACID 与回滚机制，并看 cyt 里 `@Transactional` 的真实用法和那些"事务不生效"的经典坑。

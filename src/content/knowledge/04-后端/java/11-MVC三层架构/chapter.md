...
## 三、用 cyt-basic 真实链路串一遍

我们拿第 4 课见过的 `/company/getById` 接口，把三层的真实代码摆在一起看。三个文件分别是：

- Controller：`cyt-basic/cyt-basic-biz/.../controller/CompanyController.java`
- Service：`cyt-basic/cyt-basic-service/.../service/CompanyService.java`
- Mapper：`cyt-basic/cyt-basic-service/.../mapper/CompanyMapper.java`

### 第 1 层 Controller —— 薄薄一层，只做收发

```java
@RestController                       // 声明这是个 REST 控制器，返回值自动转 JSON（见第 8 课注解）
@RequestMapping("/company")           // 该类下所有接口的统一前缀
public class CompanyController extends BaseController {

    @Autowired                        // 由 Spring 注入 Service 实例（见第 5 课依赖注入）
    private CompanyService companyService;

    @GetMapping("/getById")           // GET /company/getById?id=123
...
```java
@Slf4j
@Service("companyService")            // 声明为业务层 Bean，名字叫 companyService
public class CompanyService extends ServiceImpl<CompanyMapper, Company> {
    ...
    public CompanyOut getCompany(Integer id) {
        ...
        // ④ 实体转出参：Company 是数据库实体，CompanyOut 是对外的视图对象
        //    WHY 要转：实体里可能有不该暴露给前端的字段（如内部状态、密钥）
        CompanyOut companyOut = new CompanyOut();
        BeanUtils.copyProperties(company, companyOut);   // 同名字段自动拷贝
...
| 对象 | 角色 | 前端类比 |
|---|---|---|
| `Company` (Entity) | 跟数据库表一一对应的「原始数据」 | 后端原始 response，字段全 |
| `CompanyOut` (Out/VO) | 给前端看的「裁剪后视图」 | 你在 api 层 `transform` 后吐给组件的 DTO |

为什么不直接把 `Company` 返给前端？因为实体字段可能含敏感信息（内部状态码、结算密钥），也可能字段名和前端期望不一致。`BeanUtils.copyProperties` 就是那道「过滤+整形」的关口，等价于你在 `request.ts` 里写的 `res => ({ id: res.id, name: res.companyName })`。

#### 进阶看一眼：Service 编排能有多复杂

`getById` 只是入门款。同一个 `CompanyService` 里的 `driverCompanyTree`（查司机所属集团树形结构）才是 Service 层威力的体现——它一口气编排了多个数据源：

```java
public DrCompanyTreeOut driverCompanyTree(CompanyTreeIn companyTreeIn) {
    // 1. 先查司机关联的集团（调 driverCompanyService，另一个 Service）
    List<DriverCompany> driverCompanyList = driverCompanyService.list(...);

    // 2. 业务规则：极速版客户不能上报在途费用，过滤掉（这是纯业务知识）
    filterOutTopSpeed(driverCompanyList);
    if (CollectionUtils.isEmpty(driverCompanyList)) {
        throw new BusinessException("未查询到司机信息");
    }

    // 3. 业务规则：客户数据排前面、测试集团排后面（排序背后是业务诉求）
    // 4. 再查网点、按 group 分组、递归组装成树形结构 ...
    Map<Integer, List<Company>> groupComs =
        companyList.stream().collect(Collectors.groupingBy(Company::getGroupId));
    // ...
}
```
...
public interface CompanyMapper extends BaseMapper<Company> {
    ...
    Company selectCompanyForUpdate(@Param("id") Integer id);
}
```

`Mapper` 是个**接口**，没有方法体——这是 Java 后端一个让前端同学惊讶的点。`getCompany` 里调的 `baseMapper.selectById(id)`，方法体在哪？答案是 MyBatis-Plus 框架在运行时用动态代理（见第 8 课反射）帮你生成了实现，自动拼出类似 `SELECT id, company_name, ... FROM company WHERE id = ?` 的语句（MyBatis-Plus 会按实体字段列出具体列名，而不是 `SELECT *`）。
...
前端读者可能会问：「`getById` 就三行逻辑，分三个文件不是脱裤子放屁吗？」我们用四个真实场景回答。

### 1. 复用：业务方法被多处调用

`CompanyService.getCompany(id)` 不只被 `getById` 接口调用。翻一下同文件，`driverCompanyTree` 内部也调了它：

```java
CompanyOut company = companyService.getCompany(driverCompanyList.get(0).getCompanyId());
```
...
哪天 `company` 表要分库分表、或者改成调远程服务取数据，**只动 Mapper 层**，Service 和 Controller 一行不改。因为 Service 只依赖「`baseMapper.selectById` 能给我一个 Company」这个约定，不关心底层是 MySQL 还是别的。
...
数据库事务（要么全成功要么全回滚）应该包住「一组业务操作」，而一组业务操作恰好就是 Service 的一个方法。看真实例子：

```java
@Transactional(rollbackFor = Exception.class, timeout = 6)   // 事务注解加在 Service 方法上
public void incrementUpdateSettleRemainderById0(IncrementUpdateSettleRemainderIn updateReq) {
    Company company = baseMapper.selectCompanyForUpdate(...);  // select for update 加锁
    // ... 算新余额 ...
    baseMapper.updateById(companyUpdate);                      // 更新
...
cyt 是个多模块 Maven 工程（见第 1 课 Maven），三层物理上分在不同模块/包里：

```
cyt-basic/
├── cyt-basic-biz/                    ← 对外暴露的「业务入口」模块
│   └── .../controller/
│       └── CompanyController.java    ← Controller 层在这
│
├── cyt-basic-service/                ← 核心业务实现模块
│   └── .../service/
│   │   └── CompanyService.java       ← Service 层在这
│   └── .../mapper/
│       └── CompanyMapper.java        ← Mapper 层在这
│
└── cyt-basic-common/                 ← 共享的数据传输对象
    └── .../in/  /out/                ← CompanyIn(入参) / CompanyOut(出参)
```
...
3. **Mapper**（或 Service 里构造 QueryWrapper 时）。这是数据查询条件。cyt 里很多查询用 `Wrappers.<Company>lambdaQuery().eq(Company::getState, ...)` 在 Service 里拼条件，简单查询拼在 Service、复杂 SQL 落到 Mapper 的 XML，都可接受。
4. **Service**。用 `BeanUtils.copyProperties(company, companyOut)` 做 实体→出参 的裁剪，是业务层的数据整形职责。
...
- **Controller 要薄**：只解析参数、调 Service、用 `R.ok()` 包响应；不写业务、不碰数据库。cyt 的 `getCompany` 方法体只有一行。
- **Service 是大脑**：业务校验（抛 `BusinessException`）、流程编排（如 `driverCompanyTree` 攒多个数据源）、事务边界（`@Transactional`）、实体→出参转换（`BeanUtils.copyProperties`）都在这层。
...
- 真实链路引用：`cyt-basic` 的 `CompanyController.getCompany` → `CompanyService.getCompany` → `CompanyMapper(extends BaseMapper).selectById`，外加 `driverCompanyTree`、`incrementUpdateSettleRemainderById0` 展示了复杂编排与事务。

> **下一课预告**：第 12 课《Spring 的 IOC 与 DI》。这一课你反复看到 `@Autowired private CompanyService companyService`——这些对象到底是谁、在什么时候、怎么塞进来的？我们会拆开 Spring 容器，讲清楚「控制反转」和「依赖注入」这两个让前端同学一开始最懵、但真正理解后会拍大腿的核心概念。

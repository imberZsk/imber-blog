...
- **计算/调用昂贵**：第三方接口返回的 token（见下文 cyt 真实案例）
...
为什么必须"共享"这么重要？因为生产环境的后端服务不是一台机器。比如 cyt-basic 这个服务会同时部署好几个实例，请求被负载均衡随机分到不同实例上。如果缓存放在每个实例自己的内存里（像 JVM 里的一个 `Map`），那 A 实例缓存的数据 B 实例看不到，数据就不一致了。Redis 作为**独立的一台中心化内存库**，所有实例都连它，自然就共享了。
...
### cyt 真实案例：用 String 的 INCR 生成业务序号

cyt-vehiclebiz 里生成单号序号，用的就是 String 的原子自增能力。看 `SerialNumManager.java`：

```java
// 文件：cyt-vehiclebiz/.../manager/common/SerialNumManager.java
...
这里的 `redisKey` 由 `固定前缀_类型_日期_集团_网点` 拼出来（见同文件 `generateAndIncSeqInRedisInCompany`：`KEY_PREFIX + type + "_" + 日期 + "_" + groupId + "_" + companyId`），日期一变 key 就变，所以**每个网点每天有独立的计数器**。
...
### cyt 真实案例：网货 token 缓存

cyt-basic 里申请第三方"网货数据上报 token"是个**昂贵的 HTTP 调用**，而且 token 有有效期，所以非常适合 Cache Aside。看 `DefaultTokenApplyService.java`：

```java
// 文件：cyt-basic/.../service/wlhy/impl/DefaultTokenApplyService.java
@Autowired
private StringRedisTemplate redisTemplate;

// 网货 token 缓存键前缀，从 Apollo 配置中心读取（见第 04 课提到的配置）
@Value("${cyt.redis.key.wlhy.token:cyt-basic_wlhy_token_}")
private String wlhyTokenKey;

// 申请网货数据上报 token
public String getToken(String customerName, WlhyCustomerConf.ProvinceInfo customerProvince,
                       WlhyProvincePlatConfig provincePlatConfig) throws IOException {
    // 缓存 key 用 客户名_省份 区分，不同省份平台 token 不同
    String key = wlhyTokenKey + customerName + "_" + customerProvince.getProvince();
    ...
    if (StringUtils.isNotEmpty(token)) {
        // 关键参数 WLHY_TOKEN_EXP：过期分钟数。必须小于 token 真实有效期，
        // 否则缓存里的 token 还在、第三方那边却已失效，请求就会报错
        redisTemplate.opsForValue().set(key, token, RedisKeyConstant.WLHY_TOKEN_EXP, TimeUnit.MINUTES);
    }
    return token;
}
```
...
这三个词面试必考，业务里也真实存在。名字容易混，用一张表先分清：
...
cyt-finance 里就有现成的分布式锁能力。看 `LockServiceImpl.java`：

```java
// 文件：cyt-finance/.../service/frame/common/impl/LockServiceImpl.java
@Autowired
...
// 给已存在的 key 补设过期时间（见前面 SerialNumManager 的例子）
...
此外，当内存满了，Redis 会按**淘汰策略**（如 `allkeys-lru`：淘汰最久没用的）主动腾空间。这部分通常由运维配置，了解即可。
...
- **Cache Aside 模式**：读"先缓存后数据库再写回"，写"先更库再删缓存"。cyt 网货 token 缓存是标准范例。
- **三大问题**：穿透（查不存在→缓存空值/布隆过滤器）、击穿（热点过期→分布式锁，cyt-finance 有现成实现）、雪崩（集体过期→过期时间加随机值+高可用）。
- **过期策略**：TTL + 惰性删除 + 定期删除 + 内存淘汰策略，思想等同于 HTTP 缓存的 `max-age`。
- **cyt 真实代码**：`SerialNumManager`（String 的 INCR 生成业务序号）、`DefaultTokenApplyService`（Cache Aside 缓存第三方 token）、`LockServiceImpl`（Redis 分布式锁防击穿）。

**下一课预告**：第 26 课《消息队列入门》——我们会看后端如何用消息队列（如 RocketMQ/Kafka）做异步解耦和削峰填谷，前端类比是 EventBus 和事件队列，再结合 cyt 真实的消息发送场景讲清楚"为什么有些事不立即做、而是丢进队列慢慢做"。

...
你在前端肯定遇到过这种需求：一个搜索框，用户随便输点啥，要从几百万条运单 / 车辆使用记录里「模糊地、跨字段地、还要分页排序地」捞出结果。

如果让 MySQL 来扛，SQL 大概长这样：

```sql
SELECT * FROM truck_record
WHERE tr_num LIKE '%12345%'      -- 车牌模糊匹配
...
|--------------|-------|---------|------|
| **index**（索引） | table（表） | 一个数据集合 | 比如 cyt 里的 `TruckRecord` 索引 |
| **document**（文档） | row（行） | 一个 JS 对象 | 一条车辆使用记录就是一个 document |
| **field**（字段） | column（列） | 对象的属性 | `tr_num`、`create_time` 等 |
...
// ES mapping：不仅定义类型，还定义「怎么搜」
{
  "mappings": {
    "properties": {
      "tr_num":      { "type": "keyword" },   // keyword=整串精确匹配，不分词
...
| `keyword` | 否 | 车牌号、ID、状态枚举 | 整串作为一个 term，要么全等要么不匹配 |

---

## 五、cyt 实战：运单与车务的 ES 搜索

cyt 里 ES 是由 **cyt-tms（老的 PHP 系统）** 统一维护的，其他 Java 服务（如 `cyt-vehiclebiz` 车务）通过 **Feign 调用 tms 的搜索接口**来用 ES，自己并不直接连 ES。这是个很真实的架构——历史系统沉淀了 ES 能力，新服务复用它。

### 5.1 Feign 客户端：暴露 tms 的 ES 搜索能力

真实代码见 `cyt-vehiclebiz/.../client/tms/TmsSearchClient.java`：

```java
@FeignClient(contextId = "tmsSearchClient", name = "cyt-tms-sidecar",
        configuration = SpringFormEncoder.class)
public interface TmsSearchClient {

    /**
     * ES 查询通用搜索接口
     * 调用 tms 的 plainSearch，底层走 Elasticsearch
     *
     * @param req 搜索请求参数（fields/data_name/filter/query/sort/page）
     * @return 命中的文档列表，每个文档是一个 Map
     */
    @PostMapping(value = "/api/Table/Search/plainSearch4Sidecar",
            consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
    CytCommonRespDTO<List<Map<String, Object>>> plainSearch4Sidecar(TmsProBaseReqDTO req);
}
```
...
cyt 用一个 DTO 封装 ES 搜索参数，见 `client/tms/dto/req/BatchEsSearchInDTO.java`：

```java
@Data
public class BatchEsSearchInDTO {
    private String[] fields;                // 要返回哪些字段（≈ SQL 的 SELECT 列）
    private String dataName;                // 搜哪个 index，如 "TruckRecord"
...
### 5.3 真实业务代码：车辆使用记录的 ES 搜索

看 `service/truckRecord/TruckRecordService.java` 怎么拼这个请求（已精简，保留真实逻辑）：

```java
// 定义需要从搜索结果中提取的字段（≈ SQL SELECT 的列清单）
String[] fields = new String[] {
        "uuid", "id", "tr_id", "tr_num", "tr_num_ext", "mileage", "oil_ele_num",
...
// group_id 做租户隔离——只搜当前用户所属车队的数据，这是多租户系统的安全底线
...
baseSearchReq.put("data_name", "TruckRecord"); // 指定搜 TruckRecord 这个 index
...
do {
    ...
    // 调用 tms 的 ES 搜索，底层走 Elasticsearch
    List<Map<String, Object>> plainSearch = tmsSearchManager.plainSearch(baseSearchReq);
...
cyt 里运单系统（`cyt-waybill`）在 `infra/elasticsearch/index` 和 `repository` 下预留了 ES 模块的包结构（目前只有 `package-info.java` 占位，还没填充实现），为后续运单和日志检索留好了位置。下面这个日志场景就是 ES 的典型用武之地：

```
日志场景为什么必须用 ES？
  ┌────────────────────────────────────────┐
  │ 每天产生千万级运单操作日志              │
  │ 运维要查「昨天 14:00-15:00 运单 X 的    │
  │         所有状态变更，含错误关键词」    │
  └────────────────────────────────────────┘
            │
...
```json
{
  "data_name": "TruckRecord",
  "fields": ["tr_num", "address", "create_time"],
  "filter": {
    "group_id": 100,
    "status": 1
  },
...
- **cyt 实战**：`cyt-vehiclebiz` 通过 Feign 调 `TmsSearchClient.plainSearch4Sidecar` 复用 tms 的 ES 能力，`BatchEsSearchInDTO` 封装四件套参数，`TruckRecordService` 拼请求时强制注入 `group_id` 做租户隔离，用 `Map<String, Object>` 接动态文档再转 DTO。

> **下一课预告**：第 28 课《消息队列 MQ》——数据写完 MySQL，怎么异步同步到 ES？怎么让下单后「发短信、扣库存、记日志」互不阻塞？我们将用 cyt 里的真实 MQ 场景，讲清楚消息队列这个全栈工程师绕不开的「异步解耦神器」。

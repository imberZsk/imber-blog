...
## 一、你机器的真实环境（2026/06/09 实测）

| 项目 | 现状 | 状态 |
|------|------|------|
| JDK 版本 | JDK 8（1.8.0_492，Zulu 发行版） | ✅ 与 cyt 后端一致 |
| 编译器 javac | 1.8.0_492 | ✅ 能编译 |
| JAVA_HOME | `~/.sdkman/candidates/java/current` | ✅ 已配置 |
| Maven | 3.9.9 | ✅ cyt 用它构建 |
| 版本管理工具 | SDKMAN | ✅ 类似前端的 nvm |

**结论：环境完整，正好匹配 cyt 后端，可以直接开发。**
...
- cyt 后端（Spring Boot 2.x）就是基于 JDK 8 开发的
...
cyt 每个 Java 项目根目录都有 `pom.xml`，就是它的"package.json"。
...
- ✅ 你的环境：JDK 8 + Maven 3.9.9，用 SDKMAN 管理，匹配 cyt 后端

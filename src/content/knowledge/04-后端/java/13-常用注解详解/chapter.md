...
【前端类比】等价于 NestJS 里 `@Controller('company')` 给整个类挂一个路由前缀，并声明"我返回的是数据（JSON），不是页面"。

cyt 真实代码（`cyt-basic/.../biz/controller/CompanyController.java`）：

```java
@RestController                       // 标记：HTTP 入口类，方法返回值直接序列化成 JSON
@RequestMapping("/company")           // 类级路由前缀，下面所有方法都带 /company
...
- `@RequestMapping("/company")` 贴在**类**上做前缀；也能贴在方法上，但方法级通常用更具体的 `@GetMapping`/`@PostMapping`。
...
完整 URL = 类前缀 + 方法路径 = `/company/getById`，正是第 4 课用过的那个真实接口。
...
POST /company/getByGidWithoutDel
Body: { "ids": [1,2,3] }   → @RequestBody（整个 JSON 反序列化成对象）
```

cyt 真实代码对照（同一个 CompanyController）：
...
// ③ @RequestBody：POST 请求体里的整个 JSON 映射成 CompanyIn 对象
...
`@PathVariable` 在 cyt 这套接口里用得少（团队偏好 query/body），它长这样：

```java
// 假想写法：路径里的 {id} 绑定到方法参数
@GetMapping("/company/{id}")
public R<CompanyOut> getById(@PathVariable("id") Integer id) { ... }
```
...
cyt 真实代码（`CompanyService.java`）：

```java
@Slf4j
@Service("companyService")            // 标记为业务层 Bean，括号里是自定义 Bean 名字
public class CompanyService extends ServiceImpl<CompanyMapper, Company> {

    @Autowired
    private DriverCompanyService driverCompanyService;   // 注入另一个 Service
...
```java
// 按类型注入：容器里 CompanyService 类型只有一个，直接塞
@Autowired
private CompanyService companyService;
...
实践建议：cyt 项目里以 `@Autowired` 为主，够用且统一就好。只有"同一接口有多个实现 Bean，要指定其中一个"时，`@Resource(name=...)` 或 `@Autowired + @Qualifier` 才派上用场。
...
注意这里**没有写 `@Mapper`**——因为 cyt 在一个配置类（`MybatisPlusConfigurer`，贴了 `@Configuration`）上统一配了 `@MapperScan`，把整个包下的接口自动当成 Mapper 扫描，不必每个都贴。两种写法等价：
...
// 写法二（cyt 用的）：在配置类上一次性扫描整个包（可同时扫多个包）
@MapperScan(value = {"com.huoyunren.cyt.basic.service.mapper",
                     "com.huoyunren.cyt.common.web.mapper"})
```
...
cyt 真实代码（`CompanyService.java`，更新网点余额）：
...
public void incrementUpdateSettleRemainderById0(IncrementUpdateSettleRemainderIn updateReq) {
    ...
    Company company = baseMapper.selectCompanyForUpdate(updateReq.getCompanyId());
...
cyt 真实代码（`OilSegmentFactor.java`，四段油耗计算因子）：
...
> 小知识：`@Builder` 依赖全参构造，所以通常和 `@NoArgsConstructor` + `@AllArgsConstructor` 一起出现（这也是上面那段真实代码同时贴了 4 个注解的原因）。
...
cyt 真实代码（`FlowFinishVo.java`）里就贴了校验注解。但这里要先指出一个**真实存在的坑**：
...
> `@Valid`（Java 标准）和 `@Validated`（Spring 扩展）功能高度重叠，CompanyController 里两个都 import 了。日常用 `@Valid` 即可；`@Validated` 多用于需要"分组校验"的进阶场景。

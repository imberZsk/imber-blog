...
回顾第 04 课讲的 HTTP 请求生命周期，cyt 的一个请求会经过这五站：
...
一个请求进来，cyt 会给它分配一个**全局唯一的 ID**，叫 `traceId`（也叫 trace_id）。这个 ID 像一根线，把请求经过的所有服务、打印的所有日志全串起来。
...
cyt 用的是 **OpenTelemetry**（一套链路追踪标准）来生成和传递 traceId。来看真实工具类 `TracerIdUtils.java`：

```java
// 文件：cyt-common/cyt-common-core/.../utils/TracerIdUtils.java
...
看 cyt-basic 真实的日志配置 `logback.xml`，注意那个 `%X{...}`：

```xml
<!-- 文件：cyt-basic/cyt-basic-biz/src/main/resources/logback.xml -->
...
```
2026-06-11 10:23:01.123 [TraceId: a1b2c3d4e5f6 , SpanId: 7788] [INFO] [http-nio-8080-exec-3] c.c.c.b.CompanyController | 查询公司 id=10086
2026-06-11 10:23:01.156 [TraceId: a1b2c3d4e5f6 , SpanId: 7799] [WARN] [http-nio-8080-exec-3] c.c.c.b.CompanyService    | 公司 id=10086 不存在
```
...
登服务器后，日志通常落在一个统一的根目录下（cyt 的 logback 里用 `logger.root` 属性指定，默认 `/data/applogs`，再按服务名分子目录）。最常用的就是 `grep`。
...
grep "a1b2c3d4e5f6" /data/applogs/cyt-basic/*.log

# 2. 捞所有 ERROR 级别日志，看最近有没有异常
grep "ERROR" /data/applogs/cyt-basic/app.log | tail -50

# 3. 捞某个接口路径的请求，-A 5 表示连带显示匹配行后面 5 行（看异常栈很有用）
grep -A 5 "/company/getById" /data/applogs/cyt-basic/app.log

# 4. 实时跟踪日志，边操作边看（约等于 DevTools Network 开着不关）
tail -f /data/applogs/cyt-basic/app.log | grep "10086"
```
...
线上几十台机器，不可能挨个登上去 grep。cyt 线上日志统一输出 JSON（看上面那个 ELK appender），汇集到 **ELK**（Elasticsearch + Logstash + Kibana），通过 **Kibana** 这个 Web 界面检索。
...
# 查某个服务最近的 ERROR
app_name: "cyt-basic" and level: "ERROR"

# 组合：某服务、某时间段、含某关键词
app_name: "cyt-waybill" and message: "库存不足"
```
...
> 怎么拿到 traceId？让前端把它打到 Console，或者后端在错误响应的 header / body 里带上。cyt 的网关和响应通常会回带 traceId，前端同学 F12 一看就有。
...
404 多半是"路由没匹配上"，而不是业务错误。重点查：
  ...
  4. HTTP Method 对吗？前端用 GET，后端只写了 @PostMapping
     —— 注意：路径找到了但方法不对，标准 Spring 返回的是 405（Method Not Allowed），不是 404；
        而在 cyt 里它会被下面的全局异常处理器接住，转成 HTTP 200 + errno=1（详见下方代码）
```

【前端类比】就像 Vue Router 配了 `/user/:id` 但你跳转到 `/users/123`（多了个 s），匹配不上直接 404。后端路由同理。

cyt 里 Method 不匹配会被全局异常处理器接住——**注意它不是返回裸的 405/404，而是统一转成 HTTP 200 + `errno=1`（请求参数错误）**，看真实代码：

```java
// 文件：cyt-common/cyt-common-web/.../advice/CommonWebExceptionHandler.java
...
public ResponseEntity<CommonCodeResponse> handler(HttpRequestMethodNotSupportedException e) {
    ...
    // 注意：HTTP 状态码是 200，错误信息放在响应体的 errno/message 里（cyt 统一风格）
...
> cyt 的约定：**HTTP 状态码基本恒为 200，真正的错误码在响应体的 `errno` 字段**（`errno=0` 才是成功）。所以排查 cyt 接口别只盯着 DevTools 里的 Status，要看 Response body 里的 `errno` 和 `message`。真正出现裸 404 的场景，通常是请求压根没进到应用（网关没转发、路径前缀错），这时要去查第②站网关。
...
- **cyt 风格**：HTTP Status 仍是 `200`，但响应体 `errno=2`、`message="未知异常"`——因为 cyt 把未捕获异常统一兜底转成了 200（见下方代码）。看到"未知异常"四个字，就等价于别处的 500。
...
cyt 的全局异常处理器对"未知异常"会打 ERROR 日志并告警，看真实代码：

```java
// 文件：cyt-common/cyt-common-web/.../advice/CommonWebExceptionHandler.java
// 兜底处理所有没被专门 catch 的异常 = 别处的 500（cyt 里转成 200 + errno=2）
...
public ResponseEntity<CommonCodeResponse> handler(HttpServletRequest request, Exception e) {
    ...
    // 返回 HTTP 200，错误码 errno=2、message="未知异常"放在响应体
    return builderResponseEntity(HttpStatus.OK)
...
```
ERROR ... 未知异常:
java.lang.NullPointerException: Cannot invoke "Company.getName()" because "company" is null
    at c.c.c.b.service.CompanyService.format(CompanyService.java:88)   <-- 看这行！第88行
    at c.c.c.b.controller.CompanyController.getById(CompanyController.java:42)
    ...
```

【前端类比】这和你在浏览器里看 JS 报错堆栈一模一样——`Cannot read properties of null` 就是 JS 版的 NPE，你也是顺着堆栈点进出错的那一行。Java 的栈从上往下读，最上面那行 `at ...:88` 就是案发现场。

> 区分一下：cyt 里**业务异常**（`BusinessException`，第 07 课讲过）是预期内的错误（如"余额不足"），打的是 `WARN` 日志、返回友好提示，不算 500；只有**没预料到的异常**才走上面的 `Exception.class` 兜底，打 ERROR + 告警。排查时优先看 ERROR。
...
```
10:23:01.100 [TraceId: xxx] CompanyController | 开始查询
10:23:01.105 [TraceId: xxx] CompanyService    | 调用 mapper 查询公司列表
10:23:08.880 [TraceId: xxx] CompanyService    | 查询完成，返回 5000 条   <-- 这里！隔了7.7秒
10:23:08.885 [TraceId: xxx] CompanyController | 返回结果
```
...
数据不对 = 逻辑错，没有异常栈兜底，全靠"对比"和"分层验证"。用二分法：
  ...
  3. 再确认"Controller 返回前是什么"——看请求响应日志（cyt 有 ReqRespLogFilter 自动打印出入参）
  4. 最后对比"前端收到的是什么"——DevTools Response
  逐层定位"数据从哪一层开始变错的"
```

cyt 有个自动打印**请求出入参**的过滤器 `ReqRespLogFilter`，排查"数据不对"时它打印的响应体就是宝藏——你能看到后端到底返回了什么，从而判断是后端给错了还是前端渲染错了：

```java
// 文件：cyt-common/.../web/filter/ReqRespLogFilter.java
@Slf4j
...
- **traceId 是核心武器**：cyt 用 OpenTelemetry 生成全局唯一 ID，通过 MDC（≈请求级全局变量袋子）自动贴到每行日志（`%X{traceId}`），一个 traceId 串起跨服务的所有日志。
...
- **引用的 cyt 真实代码**：`TracerIdUtils`（traceId 获取）、`logback.xml`（日志格式与 ELK 输出）、`CommonWebExceptionHandler`（404/500 异常处理与日志）、`ReqRespLogFilter`（自动打印出入参）。

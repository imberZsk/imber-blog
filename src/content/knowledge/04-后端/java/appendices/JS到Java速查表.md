...
⚠️ **字段私有 + getter/setter 是 Java 强约定**：JS 里习惯直接 `obj.name`，Java 里字段一般 `private`，外部通过 `getName()`/`setName()` 访问。实际项目常用 Lombok 的 `@Data` 注解自动生成，你会在 cyt 代码里大量见到（详见第 08 篇注解、第 18 篇）。
...
future.thenApply(res -> parse(res));
...
| `res.json(obj)` | `return obj` | ⚠️ 直接 return，自动转 JSON |
| `express.Router()` | `@RequestMapping("/前缀")` 类级注解 | 路由分组 |
| 手写校验 | `@Valid` + 校验注解 | 参数校验，像 zod 但基于注解 |

⚠️ **不用手动 `res.json()`**：Spring 的 `@RestController` 会自动把返回的对象序列化成 JSON。直接 `return user;` 前端就收到 JSON 了。详见第 12 篇、第 19 篇。
...
| `node app.js`（直接跑源码） | `javac X.java && java X` | ⚠️ Java 要先编译再运行，详见 notes.md |
...
> 想深入某个点：语法基础回第 03~08 篇，Spring Boot 回第 09~13 篇，数据库回第 14~15 篇，cyt 真实代码导读回第 17~20 篇。

...
> List/Map/Set 对比 JS 数组/对象。cyt 里 stream 操作随处可见，正好对照 JS 数组方法。
...
cyt 代码里大量用 `stream()`，它就是 **Java 版的数组方法链**（map/filter/reduce）。

### 真实代码对照（CompanyService.java）

```java
// cyt 真实代码：取出所有 groupId 去重成 Set
Set<Integer> groupSet = driverCompanyList.stream()
        .map(DriverCompany::getGroupId)    // 取每个元素的 groupId
...
const groupSet = new Set(
  driverCompanyList.map(item => item.groupId)
)
```
...
### filter 真实例子（CompanyService.java:355）

```java
// cyt：过滤掉 id 等于 parentId 的元素
...
```javascript
companyList = companyList.filter(item => item.id !== parentId)
```

**关键差异：**
1. Java 要 `.stream()` 开头、`.collect(...)` 结尾（把流收集回 List）
2. Java 的 lambda 用 `->`（JS 用 `=>`）
3. `DriverCompany::getGroupId` 是方法引用，等价于 `item -> item.getGroupId()`

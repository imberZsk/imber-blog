...
> 用 cyt-basic 真实接口 `getById`，走完"前端请求 → 数据库 → 返回"全链路，建立后端思维模型。

## 一、前端发起的请求

```javascript
// 前端代码（你熟悉的部分）
const res = await axios.get('/company/getById?id=123')
console.log(res.data)  // { code: 0, data: { id: 123, name: "北京网点" } }
```
...
## 三、第 1 站：网关（cyt-openresty-gateway）

请求先到网关，网关看 URL 决定转发给哪个服务。`/company/...` 转发给 `cyt-basic`。
...
真实代码 `CompanyController.java:38-41`：

```java
@RestController                          // ① 声明这是 REST 接口类
@RequestMapping("/company")              // ② 负责 /company 开头的请求
public class CompanyController {

    @Autowired                           // ③ 自动注入 Service（不用 new）
    private CompanyService companyService;

    @GetMapping("/getById")              // ④ 处理 GET /company/getById
    public R<CompanyOut> getCompany(@RequestParam("id") int id) {  // ⑤ 接收 id
        return R.ok(companyService.getCompany(id));  // ⑥ 调 Service，包装返回
    }
}
```
...
| `@RequestMapping("/company")` | 管 /company 开头的请求 | `app.use('/company')` |
| `@GetMapping("/getById")` | 处理 GET /company/getById | `router.get('/getById')` |
| `@RequestParam("id")` | 取 URL 的 ?id=123 | `req.query.id` |
| `companyService.getCompany(id)` | 把活交给 Service | 调 service 函数 |
| `R.ok(...)` | 包装成统一返回格式 | `res.json({code:0, data})` |
...
真实代码 `CompanyService.java:76-90`：
...
### 关键设计：为什么有 Company 又有 CompanyOut？

```
Company    = 数据库表对应的对象（几十个字段，含内部字段）
CompanyOut = 返回给前端的对象（只挑前端需要的字段）
```

`BeanUtils.copyProperties(company, companyOut)` 把数据库对象字段拷到返回对象。

**为什么不直接返回 Company？** 数据库表可能有敏感字段（内部备注、删除标记），不该全暴露给前端。这是后端安全实践，前端没有这个概念。（详见 19 课）
...
> `selectById` 是框架内置方法，藏在父类 `ServiceImpl` 里（见 `CompanyService` 第 62 行 `extends ServiceImpl<CompanyMapper, Company>`）。详见 15 课。
...
数据库返回一行数据，框架自动装进 `Company` 对象，原路返回。
...
```
MySQL 返回一行数据
  → Mapper 装进 Company 对象
    → Service 拷贝成 CompanyOut，做业务处理
...
前端 `res.data.data` 取的就是这个 `data` 字段。（R 对象详见 20 课）

...
> 注意 URL 风格：这里用 **RESTful 风格**（资源 `/articles` + HTTP 方法表达动作），和 Java cyt 那套 `/article/add`、`/article/list`（动词放路径里）不同。两种都常见，FastAPI 社区更偏 RESTful，跟着团队规范走即可。
...
> ⚠️ 响应约定差异：FastAPI 默认**直接用 HTTP 状态码表达结果**（200 成功 / 422 校验失败 / 404 找不到），返回体就是数据本身或 `{"detail": "..."}`。这和 Java cyt 那套"一律 HTTP 200，靠 `errno` 区分成败"是两种风格。前端对接 FastAPI 时**看 HTTP 状态码 + catch 错误**即可，不用先判 `errno`。具体用哪套看团队约定。
...
- FastAPI 默认**靠 HTTP 状态码**表达成败，和 Java cyt 的 `errno` 风格不同，联调认状态码

...
## 四、流式输出：打字机效果 ≈ 前端的 ReadableStream
...
## 五、异步版：要 await 风格就用 AsyncOpenAI
...
## 六、Claude（Anthropic）SDK：思路一样，两处不同

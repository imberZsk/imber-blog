# Harness Engineering（02） - Agent Loop 与执行闭环

> 读完后，你应能完成以下任务：
> - 绘制“Harness Engineering（02） - Agent Loop 与执行闭环 / 为什么必须是"循环"，而不是"一条直线"”的关键对象与数据流，解释“看到结果之后，才知道具体要删哪几个 -> 然后逐个删（再调"删除"工具） -> 删完确认一下还有没有漏的”，并用源码位置、日志或 Trace 标注证据。
> - 为“Harness Engineering（02） - Agent Loop 与执行闭环 / 循环的四个动作：想 → 做 → 看 → 再想”设计正常与异常输入，验证“把这个循环拆开，每一圈其实就四个动作。”，输出首个偏差位置与回归测试结果。
> - 实现“Harness Engineering（02） - Agent Loop 与执行闭环 / 跟着一个真实任务走一遍循环”的最小代码或配置，检验“整个过程没有任何地方是提前写死的——读几个文件、读哪些，全是模型在每一圈现场决定的。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **为什么必须是"循环"，而不是"一条直线"**：先看看有哪些 .log 文件（要先调一个"列目录"的工具） -> 看到结果之后，才知道具体要删哪几个 -> 然后逐个删（再调"删除"工具） -> 删完确认一下还有没有漏的
- **循环的四个动作：想 → 做 → 看 → 再想**：把这个循环拆开，每一圈其实就四个动作。
- **跟着一个真实任务走一遍循环**：整个过程没有任何地方是提前写死的——读几个文件、读哪些，全是模型在每一圈现场决定的。
- **循环何时停止？（三道刹车）**：正常刹车——模型说"我说完了"。 -> 安全刹车——到达最大轮次上限。 -> 人工刹车——用户喊停。 用户按 Ctrl+C，或在交互式 Agent 里打断它。
- **把循环写成代码**：把这段代码和第一章那个"伪代码循环"对一下，你会发现核心一模一样，只是多了 max_iterations 这道刹车，和"一圈可以有多个工具调用"的细节。
- **常见误区**：指望它"一次性输出完整计划然后照做"，在面对未知环境时一定会翻车。

...
- **为什么要懂一点「原理」**：你不需要会造汽车才能开车，但懂一点「油门、刹车、方向盘」的原理，能让你开得更稳、更不容易出事。
- **模型的本质：它在玩「预测下一个词」的接龙游戏**：这是整章最重要的一节。
- **最佳实践（理解原理后的心法）**：把模型当成一个「读过海量书、但记不住你私事、还会脑补」的接龙高手。
- **常见错误（新手最容易踩的坑）**：以为它「记得」一切。

<!-- knowledge-lab-merged -->

# 动手实践：demo：用代码看懂 temperature 和 Token

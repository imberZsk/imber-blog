# Claude Code（07） - 权限模式与 Plan Mode：安全地放手

> 本章目标：理解各种权限模式，学会用 Plan Mode 先规划后执行，按风险控制放手程度。
> 学完你能：根据任务风险选对模式，复杂任务先出计划再动手，既高效又不踩坑。
...
- 权限模式 = 放手程度的方向盘：**询问 → 接受编辑 → 计划 → bypass**，由紧到松。
- 选择标准：**操作越不可逆、影响越大，越从严。**
- **Plan Mode（只读）是复杂任务利器**：先出计划、反复打磨，再执行。
- bypass 慎用，安全边界可写进 `CLAUDE.md`。

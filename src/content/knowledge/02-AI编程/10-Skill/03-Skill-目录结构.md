...
> 读完后，你应能完成以下任务：
> - 绘制“Skill（03） - Skill 目录结构 / 从「最小」到「完整」”的关键对象与数据流，解释“别被吓到——除了 SKILL.md 是必须的，其余全是按需添加。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Skill（03） - Skill 目录结构 / 逐个认识”设计正常与异常输入，验证“子目录的名字（reference、scripts 等）是约定俗成，不是硬性规定。”，输出首个偏差位置与回归测试结果。
> - 实现“Skill（03） - Skill 目录结构 / 为什么 SKILL.md 是「入口」”的最小代码或配置，检验“Claude 永远先读 SKILL.md，再决定要不要去翻其它文件。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **从「最小」到「完整」**：别被吓到——除了 SKILL.md 是必须的，其余全是按需添加。
- **逐个认识**：| 文件/目录 | 必须? | 作用 | 类比 |
- **为什么 SKILL.md 是「入口」**：Claude 永远先读 SKILL.md，再决定要不要去翻其它文件。
- **工程边界**：| SKILL.md | ✅ 必须 | 入口说明书：元数据 + 操作指引 | 一本书的「封面 + 正文」 |
- **实现机制**：所以正确的姿势是：SKILL.md 保持精简，只放「核心流程 + 指路」；

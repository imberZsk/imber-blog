# Claude Code（21） - MCP 与外部工具集成

> 读完后，你应能完成以下任务：
> - 绘制“Claude Code（21） - MCP 与外部工具集成 / 为什么需要 MCP”的关键对象与数据流，解释“MCP（Model Context Protocol，模型上下文协议） 改变了这点：它是一个标准接口，让 Claude Code 能直接连上外部工具和数据源，自己去查，而不是等你描述。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Claude Code（21） - MCP 与外部工具集成 / MCP 能接什么”设计正常与异常输入，验证“代码托管 / 协作平台（如读 GitHub issue、PR）”，输出首个偏差位置与回归测试结果。
> - 实现“Claude Code（21） - MCP 与外部工具集成 / MCP 服务器的三种连接方式”的最小代码或配置，检验“MCP 通过「MCP 服务器」提供能力。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **为什么需要 MCP**：MCP（Model Context Protocol，模型上下文协议） 改变了这点：它是一个标准接口，让 Claude Code 能直接连上外部工具和数据源，自己去查，而不是等你描述。
- **MCP 服务器的三种连接方式**：MCP 通过「MCP 服务器」提供能力。
- **它怎么和前面的能力配合**：MCP 是「对外连接」这一块拼图。
- **安全注意（重要）**：把外部返回当「不可信数据」：外部工具返回的内容里如果夹带「指令」，不要盲从。
- **常见错误**：工具越多，它选择越乱、越慢。
- **最佳实践**：按需接入：只连当前任务真正用得上的 MCP。 -> 最小权限：尤其数据库，生产只读，能不给写就不给写。 -> 凭据安全：token/密钥走环境变量或密钥管理，别进 git。 -> 外部数据当不可信：返回内容里的「指令」不盲从。

# Loop Engineering（01） - Loop Engineering 是什么

> 读完后，你应能完成以下任务：
> - 绘制“Loop Engineering（01） - Loop Engineering 是什么 / /loop、goal 与 Loop Engineering 不是一回事”的关键对象与数据流，解释“具体产品是否提供 /loop 或 /goal 斜杠命令，”，并用源码位置、日志或 Trace 标注证据。
> - 为“Loop Engineering（01） - Loop Engineering 是什么 / 可运行完整示例：一个 Loop 的最小契约”设计正常与异常输入，验证“每轮必须回答五个问题：当前状态是什么、这一轮要改变什么、动作实际产生了什么、验证器如何判断进展、什么条件下停止。”，输出首个偏差位置与回归测试结果。
> - 实现“Loop Engineering（01） - Loop Engineering 是什么 / 进度不是“工具调用变多””的最小代码或配置，检验“有效进度应与任务绑定：失败测试数量下降、复现范围缩小、根因假设被证伪、覆盖率或业务指标达到门槛。”，输出命令、结果与 Diff，并说明不适用边界。
...
`/loop` 只是触发器，不等于完成控制器。
每隔五分钟运行一次“检查 CI”能够发现状态变化，
但如果没有去重游标、停止条件和取消开关，
它会在 CI 通过后继续消耗资源。
能订阅 webhook 或任务事件时优先使用事件触发；
只有外部系统没有可靠事件时，才用轮询兜底。
...
例如“把性能优化好”没有指标和基线，不能作为 goal；
“在固定数据集和运行环境下，
P95 从 480 ms 降至 300 ms 以下，
错误率不升高”才可判定。
目标达到后必须由判定器驱动终止，不应等待模型自行宣称完成。

选择顺序很简单：只需周期查看状态时用 `/loop` 或调度器；
需要达成明确结果时先写 Goal Contract；
任务还需要多轮观察、行动、验证、预算控制和中断恢复时，
再实现完整的 Loop Engineering。
三者可以组合，但职责不能互相替代。
...
- **/loop、goal 与 Loop Engineering 不是一回事**：具体产品是否提供 /loop 或 /goal 斜杠命令，要以当前宿主的官方文档和命令面板为准；
- **可运行完整示例：一个 Loop 的最小契约**：每轮必须回答五个问题：当前状态是什么、这一轮要改变什么、动作实际产生了什么、验证器如何判断进展、什么条件下停止。
- **进度不是“工具调用变多”**：有效进度应与任务绑定：失败测试数量下降、复现范围缩小、根因假设被证伪、覆盖率或业务指标达到门槛。
- **Ralph 类循环**：Ralph 的典型做法是让新的 Agent 会话反复读取稳定规格和外部进度文件，每轮执行一个小任务并提交结果。
- **高风险边界**：部署、数据删除、迁移、密钥、付款和权限变更不能因为“还在循环中”而自动获权。

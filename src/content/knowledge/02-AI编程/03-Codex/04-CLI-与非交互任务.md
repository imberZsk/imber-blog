...
> 读完后，你应能完成以下任务：
> - 绘制“Codex（04） - CLI 与非交互任务 / 概念解释”的关键对象与数据流，解释“codex：启动交互式会话，适合边聊边改。”，并用源码位置、日志或 Trace 标注证据。
> - 为“Codex（04） - CLI 与非交互任务 / 使用示例”设计正常与异常输入，验证“适合生成固定产物，比如审查报告、迁移清单、目录说明。”，输出首个偏差位置与回归测试结果。
> - 实现“Codex（04） - CLI 与非交互任务 / 交互式任务”的最小代码或配置，检验“这会进入一个可以继续追问的会话。”，输出命令、结果与 Diff，并说明不适用边界。
...
- **使用示例**：适合生成固定产物，比如审查报告、迁移清单、目录说明。
- **适合 CLI 的任务**：让 Codex 按 JSON Schema 输出结构化结果。
- **常见错误**：目录错了，它读到的就是错项目。
- **最佳实践**：自动化里优先使用保守沙盒和 never 审批策略，让失败暴露出来，而不是让它偷偷越权。
- **错误 2：非交互任务还写得很含糊**：非交互模式下，你没有那么多机会及时纠偏，所以提示词要更明确：
